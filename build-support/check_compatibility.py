...
PUBLIC_ANNOTATIONS = ["InterfaceAudience.LimitedPrivate", "InterfaceAudience.Public"]
...
def checkout_java_tree(rev, path):
    ...
    subprocess.check_call(
        [
            ...
            "pipefail",
            "-c",
            ("git archive --format=tar %s java/ | " + 'tar -C "%s" -xf -')
            % (rev, path),
        ],
        cwd=get_repo_dir(),
    )
    ...
    os.symlink(
        os.path.join(get_repo_dir(), "thirdparty"), os.path.join(path, "thirdparty")
    )


def get_git_hash(revname):
    """Convert 'revname' to its SHA-1 hash."""
    return check_output(["git", "rev-parse", revname], cwd=get_repo_dir()).strip()
...
def build_tree(path):
    ...
    subprocess.check_call(
        ["mvn", "-DskipTests", "-Dmaven.javadoc.skip=true", "package"], cwd=java_path
    )
...
def find_client_jars(path):
    ...
    return [
        j
        for j in all_jars
        if ("-tests" not in j and "-sources" not in j and "-with-dependencies" not in j)
    ]
...
def run_java_acc(src_name, src, dst_name, dst):
    ...
    logging.info(
        "Will check compatibility between original jars:\n%s\n" + "and new jars:\n%s",
        "\n".join(src_jars),
        "\n".join(dst_jars),
    )
...
def main(argv):
    ...
    parser.add_option(
        "-f",
        "--force-download",
        dest="force_download_deps",
        help=(
            "Download dependencies (i.e. Java JAVA_ACC) even if they are "
            + "already present"
        ),
    )
    ...
    run_java_acc(src_rev, src_dir, dst_rev, dst_dir)
