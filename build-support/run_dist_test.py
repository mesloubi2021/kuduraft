...
with open(
    os.path.join(ROOT, "build-support", "java-home-candidates.txt"), "r"
) as candidates:
    JAVA_CANDIDATES = [
        x.strip() for x in candidates.readlines() if not x.startswith("#")
    ]
...
def fix_rpath_component(bin_path, path):
    ...
    """
    rel_tp = os.path.relpath(
        os.path.join(ROOT, "thirdparty/"), os.path.dirname(bin_path)
    )
...
def fix_rpath(path):
    ...
    p = subprocess.Popen(
        ["chrpath", path], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def main():
    ...
    symbolizer_path = os.path.join(
        ROOT, "thirdparty/installed/uninstrumented/bin/llvm-symbolizer"
    )
    ...
    env["SENTRY_HOME"] = glob.glob(
        os.path.join(ROOT, "thirdparty/src/apache-sentry-*")
    )[0]
