...
ISOLATE_SERVER = os.environ.get("ISOLATE_SERVER", "http://isolate.cloudera.org:4242/")
DIST_TEST_HOME = os.environ.get("DIST_TEST_HOME", os.path.expanduser("~/dist_test"))
...
def get_test_executions(options):
    ...
    p = subprocess.Popen(
        ctest_argv, stdout=subprocess.PIPE, cwd=rel_to_abs("build/latest")
    )
...
def create_archive_input(staging, execution, collect_tmpdir=False):
    """
    ...
    command = [
        "../../build-support/run_dist_test.py",
        "-e",
        "KUDU_TEST_TIMEOUT=%d" % (TEST_TIMEOUT_SECS - 30),
        "-e",
        "KUDU_ALLOW_SLOW_TESTS=%s" % os.environ.get("KUDU_ALLOW_SLOW_TESTS", 1),
        "-e",
        "KUDU_COMPRESS_TEST_OUTPUT=%s" % os.environ.get("KUDU_COMPRESS_TEST_OUTPUT", 0),
    ]
    ...
    archive_json = dict(
        args=["-i", out_isolate, "-s", out_isolate + "d"],
        dir=rel_to_abs("."),
        version=1,
    )
...
def create_task_json(
    ...
    if len(tasks) > MAX_TASKS_PER_JOB:
        logging.error(
            "Job contains %d tasks which is more than the maximum %d",
            len(tasks),
            MAX_TASKS_PER_JOB,
        )
        sys.exit(1)
...
def submit_tasks(staging, options):
    ...
    if not os.path.exists(DIST_TEST_HOME):
        logging.error(
            "Cannot find dist_test tools at path %s "
...
def run_tests(parser, options):
    ...
    for execution in executions:
        create_archive_input(staging, execution, collect_tmpdir=options.collect_tmpdir)
    run_isolate(staging)
...
def add_run_subparser(subparsers):
    ...
    p.add_argument(
        ...
        help=(
            "Optional arguments to append to the command line for all "
...
def add_loop_test_subparser(subparsers):
    ...
    p.add_argument(
        "cmd", help="the path to the test binary (e.g. build/latest/bin/foo-test)"
    )
...
def run_java_tests(parser, options):
    subprocess.check_call(
        [rel_to_abs("java/gradlew")] + GRADLE_FLAGS.split() + ["distTest"],
        cwd=rel_to_abs("java"),
    )
...
def add_java_subparser(subparsers):
    ...
    sp = p.add_subparsers()
    run_all = sp.add_parser("run-all", help="Run all of the Java tests via dist-test")
    ...
    loop.add_argument(
        ...
        dest="num_instances",
        type=int,
        help="number of test instances to start",
        metavar="NUM",
        default=100,
    )
...
def main(argv):
    ...
    p.add_argument(
        ...
        dest="collect_tmpdir",
        action="store_true",
        help="Collect the test tmpdir of failed tasks as test artifacts",
        default=False,
    )
    p.add_argument(
        "--no-wait",
