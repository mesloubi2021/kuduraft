...
_NAMESPACE_END_RE = re.compile(
    r"\s*(})|" r"\s*(U_NAMESPACE_END)|" r"\s*(HASH_NAMESPACE_DECLARATION_END)"
)
...
class IWYUOutputParser(object):
            ...
            if m:
                ...
                if section_re.groups >= 1:
                    ...
                    if (
                        ...
                        and this_filename != self.filename
                    ):
    ...
    def ParseOneRecord(self, iwyu_output, flags):
        ...
        for line in self.lines_by_section.get(self._REMOVE_SECTION_RE, []):
            ...
            if not m:
                raise FixIncludesError(
                    'line "%s" (for %s) has no line number' % (line, self.filename)
                )
        ...
        for line in self.lines_by_section.get(
            self._REMOVE_SECTION_RE, []
...
def _MarkHeaderGuardIfPresent(file_lines):
    ...
    for i in range(len(file_lines)):
        if not file_lines[i].deleted and file_lines[i].type not in [
            _COMMENT_LINE_RE,
            _BLANK_LINE_RE,
            _PRAGMA_ONCE_LINE_RE,
        ]:
...
def _CalculateLineTypesAndKeys(file_lines, iwyu_record):
    ...
    for line_number in iwyu_record.some_include_lines:
        if file_lines[line_number].type != _INCLUDE_RE:
            raise FixIncludesError(
                ...
                % (iwyu_record.filename, line_number, file_lines[line_number].line)
            )
    ...
    for line_number in iwyu_record.lines_to_delete:
        ...
        elif file_lines[line_number].type not in (_INCLUDE_RE, _FORWARD_DECLARE_RE):
            raise FixIncludesError(
...
def _ContainsBarrierInclude(file_lines, line_range):
    ...
    for line_number in range(*line_range):
        if not file_lines[line_number].deleted and _BARRIER_INCLUDES.search(
...
def _LinesAreAllBlank(file_lines, start_line, end_line):
    ...
    for line_number in range(start_line, end_line):
        if (
            not file_lines[line_number].deleted
        ...
        while end_line < len(file_lines):
            ...
            if line_info.deleted:
                ...
                if namespace_depth <= 0:
                    ...
                    while (
                        start_line > 0
                        and file_lines[start_line - 1].type == _BLANK_LINE_RE
                    ):
                        start_line -= 1
                    for line_number in range(start_line, end_line):
    ...
    while start_line < len(file_lines):
        ...
        while end_line < len(file_lines):
            ...
            if line_info.deleted:
                ...
                # And also blank lines.
                while (
                    start_line > 0 and file_lines[start_line - 1].type == _BLANK_LINE_RE
                ):
                    start_line -= 1
                for line_number in range(start_line, end_line):
                    file_lines[line_number].deleted = True
...
def _DeleteExtraneousBlankLines(file_lines, line_range):
    ...
    while (
        before_line
        and file_lines[before_line].type == _BLANK_LINE_RE
        and after_line
        and file_lines[after_line].type == _BLANK_LINE_RE
    ):
...
def _ShouldInsertBlankLine(
    decorated_move_span, next_decorated_move_span, file_lines, flags
):
    ...
    if decorated_move_span[0] != next_decorated_move_span[0]:
        ...
        while (
            next_line
            and next_line < len(file_lines)
            and file_lines[next_line].type == _COMMENT_LINE_RE
        ):
            next_line += 1
        return (
            next_line
            and next_line < len(file_lines)
            and file_lines[next_line].type in (_NAMESPACE_START_RE, None)
        )
    ...
    if (
        not flags.blank_line_between_c_and_cxx_includes
        and this_kind in [_C_SYSTEM_INCLUDE_KIND, _CXX_SYSTEM_INCLUDE_KIND]
        and next_kind in [_C_SYSTEM_INCLUDE_KIND, _CXX_SYSTEM_INCLUDE_KIND]
    ):
        return False
...
def _GetFirstNamespaceLevelReorderSpan(file_lines):
    ...
    for line_number in range(len(file_lines)):
        ...
        # If we're a 'contentful' line such as a (non-header-guard) #ifdef, bail.
            ...
            assert False, ("unknown line-info type", _LINE_TYPES.index(line_info.type))
...
def _IsThirdpartyInclude(line_info, thirdparty_include_dirs):
    ...
    return any(
        os.path.exists(os.path.join(inc_dir, inc_path))
...
def _IsMainCUInclude(line_info, filename):
    ...
    canonical_include = re.sub(
        r"(-inl\.h|\.h|\.H)$", "", line_info.key.replace('"', "")
    )
    ...
    if canonical_file in (canonical_include, canonical_include2):
        return True
    # Rule 2:
    if line_info.is_first_line_of_this_type and os.path.basename(
        canonical_file
...
def _GetLineKind(file_line, filename, flags):
    ...
    if file_line.deleted:
    ...
    elif _IsSystemInclude(file_line) and not _IsThirdpartyInclude(
        file_line, flags.thirdparty_include_dirs
    ):
        ...
        if flags.separate_project_includes and _IsSameProject(
            file_line, filename, flags.separate_project_includes
        ):
...
def _FirstReorderSpanWith(file_lines, good_reorder_spans, kind, filename, flags):
    ...
    *before* the using line!
    ...
    """
    assert kind in (
        _MAIN_CU_INCLUDE_KIND,
        _C_SYSTEM_INCLUDE_KIND,
        _CXX_SYSTEM_INCLUDE_KIND,
        _NONSYSTEM_INCLUDE_KIND,
        _PROJECT_INCLUDE_KIND,
    ...
    for reorder_span in good_reorder_spans:
        for line_number in range(*reorder_span):
            ...
            if (
                line_kind == _FORWARD_DECLARE_KIND
                and line_number > first_contentful_line
            ):
                continue
    ...
    while line_number < len(file_lines):
        if file_lines[line_number].deleted:
            ...
            seen_header_guard = True
            line_number += 1
        elif file_lines[line_number].type == _COMMENT_LINE_RE and not seen_header_guard:
...
def _DecoratedMoveSpanLines(iwyu_record, file_lines, move_span_lines, flags):
    ...
    for i in range(len(move_span_lines)):
        if not move_span_lines[i].deleted and move_span_lines[i].type in (
            _INCLUDE_RE,
            _FORWARD_DECLARE_RE,
        ):
    ...
    if m:
        ...
        contentful_list = [
            li.line for li in move_span_lines[first_contentful_line:] if not li.deleted
        ]
    ...
    if reorder_span is None:  # must be a new #include we're adding
        ...
        if kind == _FORWARD_DECLARE_KIND:
            (
                namespace_prefix,
                possible_reorder_span,
            ) = _GetFirstNamespaceLevelReorderSpan(file_lines)
            if (
                namespace_prefix
                and possible_reorder_span
                and firstline.line.startswith(namespace_prefix)
            ):
        ...
        if reorder_span is None:
            ...
            reorder_span = _FirstReorderSpanWith(
                file_lines, toplevel_reorder_spans, kind, iwyu_record.filename, flags
            )
...
def FixFileLines(iwyu_record, file_lines, flags):
    ...
    for line in iwyu_record.includes_and_forward_declares_to_add:
        ...
        decorated_span = _DecoratedMoveSpanLines(
            iwyu_record, file_lines, [line_info], flags
        )
    ...
    while line_number < len(file_lines):
        ...
        new_lines = []
        while (
            decorated_move_spans and decorated_move_spans[0][0] == current_reorder_span
        ):
            new_lines.extend(decorated_move_spans[0][3])  # the full content
            if len(decorated_move_spans) > 1 and _ShouldInsertBlankLine(
                decorated_move_spans[0], decorated_move_spans[1], file_lines, flags
            ):
                new_lines.append("")
...
def FixManyFiles(iwyu_records, flags):
    ...
    print(
        "IWYU %sedited %d files on your behalf.\n"
...
def ParseArgs(args):
    ...
    parser.add_option(
        ...
        default=False,
        help=(
            "Put a blank line between the group of C system "
    ...
    parser.add_option("--nosafe_headers", action="store_false", dest="safe_headers")
    ...
    parser.add_option(
        ...
        metavar="dir",
        help=(
            "Any includes which are found in <dir> are considered "
    ...
    parser.add_option(
        ...
        metavar="dir",
        help=(
            "The path to the root of the source tree. When processing "
...
def main(argv):
    ...
    if (
        ...
        and not flags.separate_project_includes.startswith("<")
        and not flags.separate_project_includes.endswith(os.path.sep)  # 'special' vals
        and not flags.separate_project_includes.endswith("/")
    ):
