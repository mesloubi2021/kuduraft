...
_RE_CLANG_ERROR = re.compile(r"^.+?:\d+:\d+:\s*" r"(fatal )?error:", re.MULTILINE)
...
def _run_iwyu_tool(paths):
    ...
    try:
        ...
        if (
            ...
            or _RE_CLANG_ERROR.search(output)
        ):
            crash(output)
        return output
...
def _do_iwyu(flags, paths):
    ...
    if len(records) < unfiltered_count:
        logging.info(
            "Muted IWYU suggestions on %d file(s)", unfiltered_count - len(records)
        )
...
def main(argv):
    ...
    parser.add_option(
        ...
        action="store_true",
        help=(
            "Determine the list of files to run IWYU automatically based on git. "
    ...
    parser.add_option(
        ...
        type="str",
        help=(
            "A path to dump the raw IWYU output to. This can be useful for "
            "debugging this tool."
        ),
    )
    ...
    if do_filtering:
        ...
        if len(paths) != orig_count:
            logging.info(
                "Filtered %d paths muted by configuration in iwyu.py",
                orig_count - len(paths),
            )
    else:
        ...
        if muted_paths:
            logging.warning(
                "%d selected path(s) are known to have IWYU issues:" % len(muted_paths)
            )
    ...
    if os.path.exists("Makefile"):
        ...
        logging.error(
            "No Makefile or build.ninja found in build directory %s", _BUILD_DIR
        )
        sys.exit(1)
