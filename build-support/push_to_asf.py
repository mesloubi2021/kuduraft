...
from kudu_util import check_output, Colors, confirm_prompt, get_my_email, init_logging
...
def check_apache_remote():
    ...
    try:
        url = (
            check_output(["git", "config", "--local", "--get", "remote.apache.url"])
            .strip()
            .decode("utf-8")
        )
    except subprocess.CalledProcessError:
        print(
            "No remote named 'apache'. Please set one up, for example with: ",
            file=sys.stderr,
        )
...
def check_gerrit_remote():
    ...
    try:
        url = (
            check_output(["git", "config", "--local", "--get", "remote.gerrit.url"])
            .strip()
            .decode("utf-8")
        )
    except subprocess.CalledProcessError:
        ...
        print(
            "the contributor guide (git remote add gerrit %s)." % GERRIT_URL,
            file=sys.stderr,
        )
        sys.exit(1)
...
def rev_parse(rev):
    ...
    try:
        return (
            check_output(["git", "rev-parse", rev], stderr=subprocess.STDOUT)
            .strip()
            .decode("utf-8")
        )
    except subprocess.CalledProcessError:
...
def describe_commit(rev):
    ...
    return (
        check_output(["git", "log", "--color", "-n1", "--oneline", rev])
        .strip()
        .decode("utf-8")
    )
...
def is_fast_forward(ancestor, child):
    ...
    try:
        merge_base = (
            check_output(["git", "merge-base", ancestor, child]).strip().decode("utf-8")
        )
    except:
...
def get_committer_email(rev):
    ...
    return (
        check_output(["git", "log", "-n1", "--pretty=format:%ce", rev])
        .strip()
        .decode("utf-8")
    )
...
def do_update(branch, gerrit_sha, apache_sha):
    ...
    if not is_fast_forward(apache_sha, gerrit_sha):
        ...
        print(
            "Apache revision %s is not an ancestor of gerrit revision %s"
    ...
    print(
        Colors.GREEN
        + ("%d commit(s) need to be pushed from Gerrit to ASF:" % len(commits))
        + Colors.RESET
    )
    push_sha = None
    for sha in commits:
        ...
        if committer != get_my_email():
            print(
                Colors.RED + "   !!! Committed by someone else (%s) !!!" % committer,
                Colors.RESET,
            )
    ...
    print(
        Colors.GREEN
        + "Successfully updated %s to %s" % (branch, gerrit_sha)
        + Colors.RESET
    )
    print()
...
def main():
    ...
    p = optparse.OptionParser(
        epilog=(
            "See the top of the source code for more information on the purpose of "
            + "this script."
        )
    )
    p.add_option(
        "-n", "--dry-run", action="store_true", help="Perform git pushes with --dry-run"
    )
