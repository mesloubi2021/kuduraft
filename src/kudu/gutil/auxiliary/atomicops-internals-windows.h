...
inline LONG
FastInterlockedCompareExchange(volatile LONG* ptr, LONG newval, LONG oldval) {
...
KUDU_COMPILE_ASSERT(sizeof(Atomic64) == sizeof(PVOID), atomic_word_is_atomic);
...
inline PVOID FastInterlockedCompareExchangePointer(
  ...
  return ::InterlockedCompareExchangePointer(
      const_cast<PVOID*>(ptr), newval, oldval);
}
...
inline LONGLONG FastInterlockedExchangeAdd64(
    volatile LONGLONG* ptr,
    LONGLONG increment) {
...
inline void NotImplementedFatalError(const char* function_name) {
  fprintf(
      stderr, "64-bit %s() not implemented on this platform\n", function_name);
  abort();
}
