...
// Both of these overloads call through to this AppendToMap() function. This is
// needed because inserting a duplicate key into a map does NOT overwrite the
// previous value, which was not the behavior of the split1 Split*() functions.
...
// for a maps. Instead, maps and unordered_maps need to be special cased to
// implement the desired append semantic of inserting an existing value
...
template <typename Splitter>
void AppendToImpl(
    unordered_map<string, string>* map_container,
    Splitter splitter) {
  AppendToMap(map_container, splitter);
}
...
static inline void SplitStringToIteratorAllowEmpty(
...
static inline void SplitStringWithEscapingToIterator(
    const string& src,
...
bool SplitStructuredLineInternal(
    StringPiece line,
    char delimiter,
    const char* symbol_pairs,
...
char* SplitStructuredLine(
    ...
    const char* symbol_pairs,
    vector<char*>* cols) {
  return SplitStructuredLineInternal(
      line, delimiter, symbol_pairs, cols, false);
}
...
bool SplitStringIntoKeyValues(
  ...
  string values_string(
      remains, begin_values_pos, remains.size() - begin_values_pos);
...
bool SplitStringIntoKeyValuePairs(
  ...
  for (const auto& pair : pairs) {
    string key;
    vector<string> value;
    if (!SplitStringIntoKeyValues(
            pair, key_value_delimiters, "", &key, &value)) {
