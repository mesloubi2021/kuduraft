...
// Padded POD container for atomic<int64_t>. This prevents false sharing of
// cache lines.
class Cell {
 public:
...
// This set of classes is heavily derived from JSR166e, released into the public
// domain by Doug Lea and the other authors.
//
// See:
...
//
// The Striped64 and LongAdder implementations here are simplified versions of
// what's present in JSR166e. However, the core ideas remain the same.
//
// Updating a single AtomicInteger in a multi-threaded environment can be quite
// slow:
//
...
// it uses CAS on a single base counter to store updates. However, when
// Striped64 detects contention (via a failed CAS operation), it will allocate a
// small, fixed size hashtable of Cells. A Cell is a simple POD that pads out an
// atomic<int64_t> to 64 bytes to prevent sharing a cache line.
//
// Reading the value of a Striped64 requires traversing the hashtable to
// calculate the true sum.
//
// Each updating thread uses a thread-local hashcode to determine its Cell in
// the hashtable. If a thread fails to CAS its hashed Cell, it will do a
// lightweight rehash operation to try and find an uncontended bucket. Because
// the hashcode is thread-local, this rehash affects all Striped64's accessed by
// the thread. This is good, since contention on one Striped64 is indicative of
// contention elsewhere too.
//
// The hashtable is statically sized to the nearest power of 2 greater than or
// equal to the number of CPUs. This is sufficient, since this guarantees the
// existence of a perfect hash function. Due to the random rehashing, the
// threads should eventually converge to this function. In practice, this scheme
// has shown to be sufficient.
//
// The biggest simplification of this implementation compared to JSR166e is that
// we do not dynamically grow the table, instead immediately allocating it to
// the full size. We also do not lazily allocate each Cell, instead allocating
// the entire array at once. This means we waste some additional memory in low
// contention scenarios, and initial allocation will also be slower. Some of the
// micro-optimizations were also elided for readability.
class Striped64 {
 public:
  Striped64() = default;
 ...
 protected:
  ...
  // Cells, and/or contention. See above for further explanation.
  //
