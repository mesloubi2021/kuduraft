...
//
// The files are named similarly to glog log files and use the following
// pattern:
//
// <log_dir>/<program-name>.<hostname>.<user-name>.<log-name>.<timestamp>.<sequence>.<pid>
//   log_dir:      the log_dir specified in the constructor
//   program-name: argv[0], as determined by
//   google::ProgramInvocationShortName() hostname:     the local machine
//   hostname user-name:    the current user name log-name:     the log_name
//   specified in the constructor timestamp:    the wall clock time when the log
//   file was created, in
//                 YYYYmmdd-HHMMSS fixed-length format.
//   sequence:     a sequence number which is used to disambiguate when the log
//   file is
//                 rolled multiple times within a second
//   pid:          the pid of the daemon
//
// The log implementation does not ensure durability of the log or its files in
...
class RollingLog {
 public:
  ...
  // roll threshold, a new empty log file is created. Note that this is a
