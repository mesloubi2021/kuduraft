...
SANITIZER_HOOK_ATTRIBUTE const char* __tsan_default_suppressions() {
  return
      ...
      //
      // With TSAN in clang 3.5, it's the init() function that's flagged as a
      // data race (not local_addr_space_init()), due to the former calling
      // sigfillset() on an unprotected global variable. Although init() calls
      // local_addr_space_init(), it can sometimes be eliminated from the call
      // stack by inlining or a tail-call # optimization, so adding the
      // suppression on both is necessary.
      "race:_ULx86_64_init\n"
      ...
      // ev_feed_signal and spoiled errno in ev_sighandler. Both are probably
      // noise.
      "race:ev_sighandler\n"

      // See https://github.com/google/glog/issues/80 for a general list of TSAN
      // issues in glog.
      // 1. glog's fatal signal handler isn't signal-safe -- it allocates
      // memory.
      //    This isn't great, but nothing we can do about it. See
      //    https://code.google.com/p/google-glog/issues/detail?id=191
      // 2. LOG(FATAL) from multiple threads can also end up triggering a TSAN
      // error.
      ...
      // on x86 for primitive flag types, but not for string flags, which is why
      // fLS is omitted.
      "race:fLB::\n"
      "race:fLD::\n"
      "race:fLI::\n"
      ...
      // This method in Boost's UUID library operates on static state with
      // impunity, triggering (harmless) data races in TSAN when
      // boost::uuids::random_generator instances are created across threads
      // (see kudu::ObjectIdGenerator).
      "race:boost::uuids::detail::seed_rng::sha1_random_digest_\n"

      // Squeasel uses ctx->stop_flag to synchronize stopping. It always sets it
      // with the context lock held, but sometimes reads it without the context
      // lock. This should be safe on x86, but is nonetheless flagged by TSAN.
      "race:sq_stop\n"

      // Squeasel reads and frees ctx->listening_sockets without taking any
      // locks. This may be an unsafe race.
      "race:close_all_listening_sockets\n"

      // ------------------------------------------------------------
      // Known bugs below. As these JIRAs are resolved, please remove the
      ...
      // KUDU-574: raft_consensus_quorum-test race on LocalTestPeerProxy
      // destruction
      ...
      // include any stack frame above the libcrypto lock destruction or memory
