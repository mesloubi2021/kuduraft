...
void MaintenanceManager::Shutdown() {
  ...
  if (monitor_thread_.get()) {
    ...
    // Wait for all the running and queued tasks before shutting down.
    // Otherwise, Shutdown() can remove a queued task silently. We count on
    // eventually running the queued tasks to decrement their "running" count,
    // which is incremented at the time they are enqueued.
    thread_pool_->Wait();
...
void MaintenanceManager::UnregisterOp(MaintenanceOp* op) {
  {
    ...
    if (iter->first->running_ > 0) {
      VLOG_AND_TRACE("maintenance", 1)
          << LogPrefix() << "Waiting for op " << op->name()
...
void MaintenanceManager::RunSchedulerThread() {
  ...
  while (true) {
    ...
    if (shutdown_) {
      VLOG_AND_TRACE("maintenance", 1)
          << LogPrefix() << "Shutting down maintenance manager.";
      return;
    }
    ...
    // schedule. Otherwise, we can go right into trying to find the next op.
    prev_iter_found_no_work = (op == nullptr);
    if (!op) {
      VLOG_AND_TRACE("maintenance", 2)
          << LogPrefix() << "No maintenance operations look worth doing.";
      continue;
    }
    ...
    if (!ready) {
      LOG_WITH_PREFIX(INFO)
          << "Prepare failed for " << op->name() << ".  Re-running scheduler.";
      op->running_--;
      running_ops_--;
      op->cond_->Signal();
      continue;
    ...
    LOG_AND_TRACE("maintenance", INFO)
        << LogPrefix() << "Scheduling " << op->name() << ": " << note;
    // Run the maintenance operation.
    Status s = thread_pool_->SubmitFunc(
        boost::bind(&MaintenanceManager::LaunchOp, this, op));
    CHECK(s.ok());
  }
}
...
// amount of resources we hold on to. Low IO Ops go first since we can quickly
// run them, then we can look at memory usage. Reversing those can starve the
// low IO Ops when the system is under intense memory pressure.
//
// In the third priority we're at a point where nothing's urgent and there's
// nothing we can run quickly.
// TODO We currently optimize for freeing log retention but we could consider
// having some sort of sliding priority between log retention and RAM usage. For
// example, is an Op that frees 128MB of log retention and 12MB of RAM always
// better than an op that frees 12MB of log retention and 128MB of RAM? Maybe a
// more holistic approach would be better.
