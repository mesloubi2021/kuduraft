...
//   For example:
//
//    VLOG_AND_TRACE("my_subsystem", 1) << "This always shows up in trace
//    buffers "
//        << "but only shows up in the log if VLOG(1) level logging is
//        enabled.";
//
...
// - We make use of the fact that a block can be embedded within a ternary
// expression.
//   This allows us to determine whether the trace event is enabled before we
//   decide to evaluate the arguments.
// - We have to use google::LogMessageVoidify so that we can put 'void(0)' on
// one side
  ...
  class TraceLogSink : public google::LogSink {
   public:
    ...
    void send(
      ...
      // --trace_to_console is enabled, then we'd end up calling back into glog
      // when its lock is already held. glog isn't re-entrant, so that causes a
      // crash.
      //
