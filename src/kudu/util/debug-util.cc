...
bool StackTraceCollector::RevokeSigData() {
  ...
  // 2) The signal was delivered. Either the thread is currently collecting its
  // stack
...
Status StackTraceCollector::TriggerAsync(int64_t tid, StackTrace* stack) {
  ...
  // accidentally send a signal to some other process in the case that the
  ...
  // The signal is now pending to the target thread. We don't store it in a
...
string StackTrace::Symbolize() const {
  ...
  for (int i = 0; i < num_frames_; i++) {
    ...
    // annotated 'noreturn', this address may actually be the first instruction
    // of the next function, because the function we care about ends with the
    // 'call'. So, we subtract 1 from 'pc' so that we're pointing at the 'call'
    ...
    // subtracting 1, we end up with 0x400448, which is still within 'main'.
    //
    // This also ensures that we point at the correct line number when using
    // addr2line on logged stacks.
    //
