...
struct SaslMechanism {
  enum Type { INVALID, PLAIN, GSSAPI };
...
// appname: Name of the application for logging messages & sasl plugin
// configuration.
//          Note that this string must remain allocated for the lifetime of the
//          program.
// This function must be called before using SASL.
// If the library initializes without error, calling more than once has no
// effect.
//
// Some SASL plugins take time to initialize random number generators and other
// things, so the first time this function is invoked it may execute for several
// seconds. After that, it should be very fast. This function should be invoked
// as early as possible in the application lifetime to avoid SASL initialization
// taking place in a performance-critical section.
//
...
// Initialize and return a libsasl2 callback data structure based on the passed
// args. id: A SASL callback identifier (e.g., SASL_CB_GETOPT). proc: A C-style
// callback with appropriate signature based on the callback id, or NULL.
...
//
// The plaintext data must not be longer than the negotiated maximum buffer
// size.
//
...
//
// The decoded plaintext must not be longer than the negotiated maximum buffer
// size.
//
