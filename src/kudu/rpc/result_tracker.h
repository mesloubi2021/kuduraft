...
//
// The ResultTracker is responsible for tracking the results of RPCs and making
// sure that client calls with the same client ID and sequence number (first
// attempt and subsequent retries) are executed exactly once.
//
// In most cases, the use of ResultTracker is internal to the RPC system: RPCs
// are tracked when they first arrive, before service methods are called, and
// calls to ResultTracker to store responses are performed internally by
// RpcContext. The exception is when an RPC is replicated across multiple
// servers, such as with writes, in which case direct interaction with the
// result tracker is required so as to cache responses on replicas which did not
// receive the RPC directly from the client.
//
// Throughout this header and elsewhere we use the following terms:
//
// RPC - The operation that a client or another server wants to execute on this
// server. The client
//       might attempt one RPC many times, for instance if failures or timeouts
//       happen.
// Attempt - Each individual attempt of an RPC on the server.
// Handler - A thread executing an attempt. Usually there is only one handler
// that executes the
//           first attempt of an RPC and, when it completes, replies to its own
//           attempt and to all other attempts that might have arrived after it
//           started.
// Driver - Only important in cases where there might be multiple handlers (e.g.
// in replicated
//          RPCs). In these cases there might be two handlers executing the same
//          RPC, corresponding to different attempts. Since the RPC must be
//          executed exactly once, only one of the handlers must be selected as
//          the "driver" and actually perform the operation.
//
// If a client wishes to track the result of a given RPC it must send on the RPC
// header a RequestId with the following information:
//
//       Client ID - Uniquely identifies a single client. All the RPCs
//       originating from the same
//                   client must have the same ID.
// Sequence number - Uniquely identifies a single RPC, even across retries to
// multiple servers, for
//                   replicated RPCs. All retries of the same RPC must have the
//                   same sequence number.
//  Attempt number - Uniquely identifies each retry of the same RPC. All retries
//  of the same RPC
...
// server has heard of the RPC and that the corresponding server function should
// be executed.
//
// If anything other than NEW is returned it means that the call has either
// previously completed or is in the process of being executed. In this case the
// caller should _not_ execute the function corresponding to the RPC. The
// ResultTracker itself will take care of responding to the client
// appropriately. If the RPC was already completed, the ResultTracker replies to
// the client immediately. If the RPC is still ongoing, the attempt gets
...
// the client a chance to either retry (if the failure reason is transient) or
// give up.
//
// ============================================================================
...
// ============================================================================
//
// Some RPCs results are tracked by single server, i.e. they correspond to the
// modification of an unreplicated resource and are unpersisted. For those no
// additional care needs to be taken, the first attempt will be the only
// handler, and subsequent attempts will receive the response when that first
// attempt is done. However some RPCs are replicated across servers, using
// consensus, and thus can have multiple handlers executing different attempts
// at the same time, e.g. one handler from a client originating retry, and one
// from a previous leader originating update.
//
...
// - Each handler replies to their own RPCs, to avoid races. That is, a live
// handler should
...
// one of the handlers will be marked as the driver. For instance, for writes,
// this serialization point is in TransactionDriver, in a synchronized block
// where a logic such as this one happens (here in pseudo-ish code):
//
// {
...
class ResultTracker : public RefCountedThreadSafe<ResultTracker> {
 public:
  ...
  // the same response, but doesn't actually store the response. This should be
  // called when the RPC failed validation or if some transient error occurred.
  ...
  //   requests are in progress, GCs the ClientState and all its
  //   CompletionRecords.
 ...
 private:
  ...
  struct ClientState {
    ...
    // single method that GCs and releases the memory for CompletionRecords
