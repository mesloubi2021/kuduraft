...
ResultTracker::RpcState ResultTracker::TrackRpcUnlocked(
  ...
  if (PREDICT_TRUE(result.second)) {
    ...
    if (response != nullptr) {
      completion_record->ongoing_rpcs.push_back(
  ...
  switch (completion_record->state) {
    ...
    case RpcState::IN_PROGRESS: {
      ...
      if (context != nullptr) {
        completion_record->ongoing_rpcs.push_back(
            {DCHECK_NOTNULL(response), context, NO_HANDLER});
      }
...
void ResultTracker::FailAndRespond(
  ...
  auto func = [&](const OnGoingRpcInfo& ongoing_rpc) {
    ...
    ongoing_rpc.context->call_->RespondApplicationError(
...
void ResultTracker::ClientState::GCCompletionRecords(
  ...
  for (auto iter = completion_records.begin();
       iter != completion_records.end();) {
