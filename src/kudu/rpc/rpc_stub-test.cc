...
TEST_F(RpcStubTest, TestCallWithMissingPBFieldClientSide) {
  ...
  p.AddAsync(
      req, &resp, &controller, boost::bind(&DoIncrement, &callback_count));
...
TEST_F(RpcStubTest, TestResponseWithMissingField) {
  ...
  ASSERT_STR_CONTAINS(
      s.ToString(), "invalid RPC response, missing fields: response");
}

// Test case where the server responds with a message which is larger than the
// maximum configured RPC message size. The server should send the response, but
// the client will reject it.
TEST_F(RpcStubTest, TestResponseLargerThanFrameSize) {
  ...
  TestInvalidResponseResponsePB resp;
  req.set_error_type(
      rpc_test::TestInvalidResponseRequestPB_ErrorType_RESPONSE_TOO_LARGE);
  Status s = p.TestInvalidResponse(req, &resp, &rpc);
...
TEST_F(RpcStubTest, TestCallMissingMethod) {
  ...
  ASSERT_TRUE(s.IsRemoteError()) << "Bad status: " << s.ToString();
  ASSERT_STR_CONTAINS(
      s.ToString(), "with an invalid method name: DoesNotExist");
}
...
TEST_F(RpcStubTest, TestDontHandleTimedOutCalls) {
  ...
  // the test.
  const Histogram* queue_time_metric =
  ...
  // because it'll get stuck in the queue for longer than its timeout.
  ASSERT_EVENTUALLY([&]() {
    RpcController rpc;
    SleepRequestPB req;
    SleepResponsePB resp;
  ...
  ASSERT_EVENTUALLY([&] { ASSERT_EQ(1, timed_out_in_queue->value()); });
}

// Test which ensures that the RPC queue accepts requests with the earliest
// deadline first (EDF), and upon overflow rejects requests with the latest
// deadlines.
//
// In particular, this simulates a workload experienced with Impala where the
// local impalad would spawn more scanner threads than the total number of
// handlers plus queue slots, guaranteeing that some of those clients would see
// SERVER_TOO_BUSY rejections on scan requests and be forced to back off and
// retry.  Without EDF scheduling, we saw that the "unlucky" threads that got
// rejected would likely continue to get rejected upon retries, and some would
// be starved continually until they missed their overall deadline and failed
// the query.
//
// With EDF scheduling, the retries take priority over the original requests
    ...
    threads.emplace_back([&, thread_id] {
      ...
      while (!done.load()) {
        ...
        for (int attempt = 1; !done.load(); attempt++) {
          ...
          // We expect to get SERVER_TOO_BUSY errors because we have more
          // clients than the server has handlers and queue slots. No other
          // errors are expected.
          CHECK(
              s.IsRemoteError() &&
...
TEST_F(RpcStubTest, TestDumpCallsInFlight) {
  ...
  ASSERT_EQ(
      "Sleep",
      dump_resp.outbound_connections(0)
          .calls_in_flight(0)
          .header()
          .remote_method()
          .method_name());
  ASSERT_GT(
      dump_resp.outbound_connections(0).calls_in_flight(0).micros_elapsed(), 0);

  // And the server messenger.
  // We have to loop this until we find a result since the actual call is sent
  // asynchronously off of the main thread (ie the server may not be handling it
  // yet)
  for (int i = 0; i < 100; i++) {
    dump_resp.Clear();
  ...
  ASSERT_EQ(
      "Sleep",
      dump_resp.inbound_connections(0)
          .calls_in_flight(0)
          .header()
          .remote_method()
          .method_name());
  ASSERT_GT(
      dump_resp.inbound_connections(0).calls_in_flight(0).micros_elapsed(), 0);
  ASSERT_STR_CONTAINS(
      dump_resp.inbound_connections(0).calls_in_flight(0).trace_buffer(),
...
TEST_F(RpcStubTest, TestDumpSampledCalls) {
  ...
  server_messenger_->rpcz_store()->DumpPB(
...
TEST_F(RpcStubTest, TestCallbackClearedAfterRunning) {
  ...
  AddResponsePB resp;
  p.AddAsync(
      req, &resp, &controller, boost::bind(MyTestCallback, &latch, my_refptr));
  latch.Wait();
...
// due to a process-wide pause or a slow callback) then we should not cause RPC
// calls to time out.
TEST_F(RpcStubTest, DontTimeOutWhenReactorIsBlocked) {
  ...
  //
  // This will cause us the reactor to be blocked while the call response is
  // received, and still be blocked when the timeout would normally occur.
  // Despite this, the call should not time out.
  //
  //  0s         0.5s          1.2s     1.5s
  //  RPC call running
  //  |---------------------|
  ...
  client_messenger_->ScheduleOnReactor(
      [](const Status& s) {
