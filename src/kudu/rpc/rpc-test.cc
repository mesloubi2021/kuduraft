...
TEST_F(TestRpc, TestSockaddr) {
  ...
  ASSERT_TRUE(
      string("0.0.0.0:1000") == addr1.ToString() ||
      string("[::]:1000") == addr1.ToString());
  ASSERT_TRUE(
      string("0.0.0.0:2000") == addr2.ToString() ||
      string("[::]:2000") == addr2.ToString());
  Sockaddr addr3(addr1);
  ASSERT_TRUE(
      string("0.0.0.0:1000") == addr3.ToString() ||
...
TEST_P(TestRpc, TestNegotiationDeadlock) {
  ...
  // ourself is just the easiest way to reproduce the issue, because the server
...
TEST_P(TestRpc, TestRpcSidecarLimits) {
  ...
  // Test two cases:
  // 1) The RPC has maximal size and exceeds rpc_max_message_size. This tests
  // the
  //    functionality of rpc_max_message_size. The server will close the
  //    connection immediately.
  // 2) The RPC has maximal size, but rpc_max_message_size has been set to a
  // higher
  ...
  for (int64_t rpc_max_message_size_val : rpc_max_message_values) {
    ...
    ASSERT_TRUE(status.IsNetworkError())
        << "Unexpected error: " << status.ToString();
...
TEST_P(TestRpc, TestCallTimeout) {
  ...
  // ASAN failures before.
  ASSERT_NO_FATAL_FAILURE(
      DoTestExpectTimeout(p, MonoDelta::FromNanoseconds(1)));

  // Test a longer timeout - expect this will time out after we send the
  // request, but shorter than our threshold for two-stage timeout handling.
  ASSERT_NO_FATAL_FAILURE(
      DoTestExpectTimeout(p, MonoDelta::FromMilliseconds(200)));
...
// followed by one with a long timeout. The call with the long timeout should
// succeed even though the previous one failed.
//
// This is a regression test against prior behavior where the connection
// negotiation was assigned the timeout of the first call on that connection.
...
TEST_P(TestRpc, TestCallTimeoutDoesntAffectNegotiation) {
  ...
  ASSERT_NO_FATAL_FAILURE(
      DoTestExpectTimeout(p, MonoDelta::FromMilliseconds(50)));
  ASSERT_OK(DoTestSyncCall(p, GenericCalculatorService::kAddMethodName));
...
// This function loops for 40 iterations and for each iteration, sends an async
