...
// An RpcSidecar is a mechanism which allows replies to RPCs to reference blocks
// of data without extra copies. In other words, whenever a protobuf would have
// a large field where additional copies become expensive, one may opt instead
// to use an RpcSidecar.
//
...
//
// After reconstructing the array of sidecars, servers and clients may retrieve
// the sidecar data through the RpcContext or RpcController interfaces
// respectively.
class RpcSidecar {
 public:
