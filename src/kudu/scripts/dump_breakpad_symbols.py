...
def parse_args():
    ...
    parser.add_argument(
        "--dump_syms", help="Path to the dump_syms binary from Breakpad"
    )
    # Options controlling how to find input files.
    parser.add_argument(
        "-b",
        "--build_dir",
        help="""Path to a directory containing results
    ...
    parser.add_argument(
        ...
        "--pkg",
        "--rpm",
        help="""RPM/DEB file containing the binaries
...
def process_binary(dump_syms, binary, out_dir):
    ...
    try:
        ...
        proc = subprocess.Popen(
            args, stdout=os.fdopen(tmp_fd, "wb"), stderr=subprocess.PIPE
        )
        _, stderr = proc.communicate()
        if proc.returncode != 0:
            sys.stderr.write(
                "Failed to dump symbols from %s, return code %s\n"
