...
def histogram_stats(aggregated_prev, aggregated_cur, m):
    ...
    """
    if (
        m not in aggregated_prev
        or m not in aggregated_cur
        or not isinstance(aggregated_cur, dict)
    ):
    ...
    # snapshots.
    delta_total = sum([val for _, val in list(cur.items())]) - sum(
        [val for _, val in list(prev.items())]
    )
...
def cache_hit_ratio(aggregated_prev, aggregated_cur):
    ...
    """
    delta_hits = delta(
        aggregated_prev, aggregated_cur, "server.block_cache_hits_caching"
    )
    delta_misses = delta(
        aggregated_prev, aggregated_cur, "server.block_cache_misses_caching"
    )
...
def process(aggregated_prev, aggregated_cur):
    ...
    calc_vals.extend(
        old_div((delta(aggregated_prev, aggregated_cur, metric)), delta_ts)
        for metric, _ in RATE_METRICS
    )
    for metric, _ in HISTOGRAM_METRICS:
        stats = histogram_stats(aggregated_prev, aggregated_cur, metric)
        calc_vals.extend(
            [stats["p50"], stats["p95"], stats["p99"], stats["p999"], stats["max"]]
        )

    print(
        old_div((aggregated_cur["ts"] + aggregated_prev["ts"]), 2),
        cache_ratio,
...
def main(argv):
    ...
    for path in sorted(argv[1:]):
        ...
        for line_number, line in enumerate(f, start=1):
            ...
            if ts <= prev_ts:
                raise Exception(
                    "timestamps must be in ascending order (%f <= %f at %s:%d)"
