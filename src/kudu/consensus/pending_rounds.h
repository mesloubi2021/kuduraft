...
//
// TODO(todd): this class inconsistently uses the term "round", "op", and
// "transaction". We should consolidate to "round".
class PendingRounds {
 public:
  PendingRounds(
  ...
  // operations with indexes higher than 'index' those operations are aborted.
  ...
  // Used by replicas to cancel pending transactions. Pending transaction are
  // those that have completed prepare/replicate but are waiting on the LEADER's
  // commit to complete. This does not cancel transactions being applied.
