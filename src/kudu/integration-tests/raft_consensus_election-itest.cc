...
class RaftConsensusElectionITest : public RaftConsensusITestBase {
 protected:
  void CreateClusterForChurnyElectionsTests(
...
void RaftConsensusElectionITest::CreateClusterForChurnyElectionsTests(
  ...
  ts_flags.insert(
      ts_flags.end(), extra_ts_flags.cbegin(), extra_ts_flags.cend());

  NO_FATALS(CreateCluster("raft_consensus-itest-cluster", ts_flags, {}));
}

void RaftConsensusElectionITest::DoTestChurnyElections(
...
// notifications from the queue back to consensus. This previously reproduced
// bugs like KUDU-1078 which normally only appear under high load.
...
TEST_F(RaftConsensusElectionITest, AutomaticLeaderElection) {
  ...
  for (int leaders_killed = 0; leaders_killed < kFinalNumReplicas;
       leaders_killed++) {
    LOG(INFO) << Substitute(
        "Writing data to leader of $0-node config ($1 alive)...",
        FLAGS_num_replicas,
        FLAGS_num_replicas - leaders_killed);
    ...
    if (leaders_killed < kNumLeadersToKill) {
      LOG(INFO) << "Killing current leader "
                << leader->instance_id.permanent_uuid() << "...";
  ...
  NO_FATALS(AssertAllReplicasAgree(
      FLAGS_client_inserts_per_thread * kFinalNumReplicas));
}
...
TEST_F(RaftConsensusElectionITest, AutomaticLeaderElectionOneReplica) {
  ...
  ASSERT_OK(GetReplicaStatusAndCheckIfLeader(
...
TEST_F(RaftConsensusElectionITest, LeaderStepDown) {
  ...
  const vector<string> kTsFlags = {"--enable_leader_failure_detection=false"};
  ...
  ASSERT_TRUE(s.IsIllegalState())
      << "TS #0 should not be leader yet: " << s.ToString();
  ...
  ASSERT_TRUE(s.IsIllegalState())
      << "TS #0 should not be leader anymore: " << s.ToString();
  ASSERT_EQ(TabletServerErrorPB::NOT_THE_LEADER, error.code())
      << SecureShortDebugString(error);
  ...
  s = WriteSimpleTestRow(
      ...
      kTimeout);
  ASSERT_TRUE(s.IsIllegalState())
      << "TS #0 should not accept writes as follower: " << s.ToString();
}
...
TEST_F(RaftConsensusElectionITest, StepDownWithSlowFollower) {
  const vector<string> kTsFlags = {
      ...
      // Bump up the RPC timeout, so that we can verify that the stepdown
      // responds quickly even when an outbound request is hung.
  ...
  ASSERT_OK(
      WaitUntilLeader(tservers[0], tablet_id_, MonoDelta::FromSeconds(10)));
  ...
  // heartbeats to the paused followers before requesting the stepdown.
...
// leader that was not in its configuration when it died. That should also work,
// i.e. the revived node should accept writes from the new leader.
TEST_F(RaftConsensusElectionITest, ElectPendingVoter) {
  ...
  //     configuration, but not the original 4-node configuration) will be
  //     available.
  ...
  ASSERT_OK(WaitUntilCommittedOpIdIndexIs(
      1, initial_leader, tablet_id_, MonoDelta::FromSeconds(10)));
  ...
  ASSERT_OK(RemoveServer(
      initial_leader, tablet_id_, final_leader, MonoDelta::FromSeconds(10)));
  ...
  for (int i = 1; i <= 3; i++) {
    ExternalTabletServer* replica_ts =
        cluster_->tablet_server_by_uuid(tservers[i]->uuid());
    ASSERT_OK(replica_ts->Pause());
  }

  // Now add server 4 back to the peers.
  // This operation will time out on the client side.
  LOG(INFO) << "Adding back Peer " << final_leader->uuid()
            << " and expecting timeout...";
  Status s = AddServer(
      initial_leader,
      tablet_id_,
      final_leader,
  ...
  ASSERT_TRUE(s.IsTimedOut())
      << "Expected AddServer() to time out. Result: " << s.ToString();
  ...
  ASSERT_OK(
      StartElection(final_leader, tablet_id_, MonoDelta::FromSeconds(10)));
  ASSERT_OK(WaitForServersToAgree(
      MonoDelta::FromSeconds(10),
      active_tablet_servers,
      tablet_id_,
...
TEST_F(RaftConsensusElectionITest, TombstonedVoteAfterFailedTabletCopy) {
  ...
  ASSERT_OK(WaitUntilCommittedConfigNumVotersIs(
      2, tablet_servers_[leader_uuid], tablet_id_, kTimeout));
  ...
  const int follower_idx = cluster_->tablet_server_index_by_uuid(follower_uuid);
  ...
  ASSERT_OK(cluster_->SetFlag(
      ...
      "tablet_copy_fault_crash_on_fetch_all",
      "1.0"));
  ...
  ASSERT_OK(cluster_->tablet_server_by_uuid(follower_uuid)
  ...
  ASSERT_OK(inspect_->CheckTabletDataStateOnTS(
      follower_idx, tablet_id_, {TABLET_DATA_COPYING}));
