...
// belonging in the InternalMiniCluster / ExternalMiniCluster classes
// themselves. But consider just putting stuff like that in those classes.
#pragma once
...
// Returns possibly the simplest imaginable schema, with a single int key
// column.
...
enum WaitForLeader { DONT_WAIT_FOR_LEADER = 0, WAIT_FOR_LEADER = 1 };
...
// Returns:
// Status::OK() if the replica is alive and leader of the consensus
// configuration. Status::NotFound() if the replica is not part of the consensus
// configuration or is dead. Status::IllegalState() if the replica is live but
// not the leader.
...
Status BulkChangeConfig(
    ...
    const std::string& tablet_id,
    const std::vector<
        consensus::BulkChangeConfigRequestPB ::ConfigChangeItemPB>& changes,
    const MonoDelta& timeout,
    const boost::optional<int64_t>& cas_config_index = boost::none,
    tserver::TabletServerErrorPB::Code* error_code = nullptr);
...
// Get the list of tablet locations for all tablets in the specified table from
// the Master.
