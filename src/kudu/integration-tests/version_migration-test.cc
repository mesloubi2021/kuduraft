...
DEFINE_string(
    test_version_a_bin,
    "",
    "Directory to find the binaries for \"Version A\"");
DEFINE_string(
    test_version_b_bin,
    "",
    "Directory to find the binaries for \"Version B\"");
 ...
 protected:
  void SetUp() override {
    if (FLAGS_test_version_a_bin.empty() || FLAGS_test_version_b_bin.empty()) {
      FAIL()
          << "Must specify --test_version_a_bin and --test_version_b_bin flags";
    }
    KuduTest::SetUp();
  }
...
void VersionMigrationTest::StartCluster(
    const vector<string>& extra_ts_flags,
    const vector<string>& extra_master_flags,
  ...
  if (!bin_path.empty())
    opts.daemon_bin_path = bin_path;
  opts.num_tablet_servers = num_tablet_servers;
  opts.extra_master_flags = extra_master_flags;
  opts.extra_master_flags.emplace_back(
      "--undefok=unlock_experimental_flags,unlock_unsafe_flags");
...
TEST_F(VersionMigrationTest, TestLinkedListSimpleMigration) {
  LOG(INFO) << "Starting cluster using binaries at "
            << FLAGS_test_version_a_bin;
  ...
  cluster_->Shutdown();
  LOG(INFO) << "Restarting cluster using binaries at "
            << FLAGS_test_version_b_bin;
