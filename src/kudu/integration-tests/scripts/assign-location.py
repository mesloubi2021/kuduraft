...
def acquire_advisory_lock(fpath):
    ...
    if not got_lock:
        raise Exception(
            "could not obtain exclusive lock for {} in {} seconds",
            fpath_lock_file,
            timeout_seconds,
        )
...
def get_location(fpath, rule, uid, relaxed):
    ...
    * If the sequencer's state file does not exist:
    ...
    if not relaxed and rule.total_count != 0 and rule.total_count <= seq:
        raise Exception(
            "too many unique identifiers ({}) to assign next location "
            "using mapping rules {}".format(seq + 1, rule.location_mapping_rules)
        )
...
def main():
    ...
    parser.add_argument(
        "uid", help="hostname, IP address, or any other unique identifier"
    )
    args = parser.parse_args()

    location = get_location(
        args.state_store,
        LocationAssignmentRule(args.location_mapping_rules),
        args.uid,
        args.relaxed,
    )
    print(location)
