...
class RaftConsensusITest : public RaftConsensusITestBase {
 public:
  ...
  // Brings Chaos to a MiniTabletServer by introducing random delays. Does this
  // by pausing the daemon a random amount of time.
  void DelayInjectorThread(
      ExternalTabletServer* tablet_server,
      int timeout_msec);
  ...
  // Stops the current leader of the configuration, runs leader election and
  // then brings it back. Before stopping the leader this pauses all follower
  // nodes in regular intervals so that we get an increased chance of stuff
  // being pending.
  void StopOrKillLeaderAndElectNewOne();

  // Writes 'num_writes' operations to the current leader. Each of the
  // operations has a payload of around 128KB. Causes a gtest failure on error.
  void Write128KOpsToLeader(int num_writes);

  // Ensure that a majority of servers is required for elections and writes.
  // This is done by pausing a majority and asserting that writes and elections
  // fail, then unpausing the majority and asserting that elections and writes
  // succeed. If fails, throws a gtest assertion. Note: This test assumes all
  // tablet servers listed in tablet_servers are voters.
  void AssertMajorityRequiredForElectionsAndWrites(
...
int64_t RaftConsensusITest::GetTimestampOnServer(
    TServerDetails* tserver) const {
  int64_t ret;
...
void RaftConsensusITest::WaitForRowCount(
    TabletServerServiceProxy* replica_proxy,
    int expected_count,
    vector<string>* results) {
  ...
  FAIL() << "Did not reach expected row count " << expected_count << " after "
         << (MonoTime::Now() - start).ToString() << ": rows: " << *results;
}

void RaftConsensusITest::AddOp(
    const OpId& id,
    int64_t base_ts,
    ConsensusRequestPB* req) {
  AddOpWithTypeAndKey(
      ...
      id.index() * 10000 + id.term(),
      req);
}

void RaftConsensusITest::AddOpWithTypeAndKey(
    const OpId& id,
    int64_t base_ts,
    RowOperationsPB::Type op_type,
    int32_t key,
  ...
  AddTestRowToPB(
      ...
      key,
      id.term(),
      SecureShortDebugString(id),
      write_req->mutable_row_operations());
}

string RaftConsensusITest::DumpToString(
    TServerDetails* leader,
    const vector<string>& leader_results,
    TServerDetails* replica,
    const vector<string>& replica_results) {
  string ret = Substitute(
      "Replica results did not match the leaders."
...
void RaftConsensusITest::InsertPayloadIgnoreErrors(
    int start_row,
    int num_rows,
    int payload_size) {
...
void RaftConsensusITest::DelayInjectorThread(
  ...
  while (inserters_.count() > 0) {
    // Adjust the value obtained from the normalized gauss. dist. so that we
    // steal the lock longer than the the timeout a small (~5%) percentage of
    // the times. (95% corresponds to 1.64485, in a normalized (0,1) gaussian
    // distribution).
    double sleep_time_usec =
        1000 * ((random_.Normal(0, 1) * timeout_msec) / 1.64485);
    ...
    LOG_IF(INFO, sleep_time_usec > 0.0)
        << "Delay injector thread for TS "
        << tablet_server->instance_id().permanent_uuid()
...
void RaftConsensusITest::StubbornlyWriteSameRowThread(
    int replica_idx,
  ...
  // Manually construct an RPC to our target replica. We expect most of the
  // calls to fail either with an "already present" or an error because we are
  // writing to a follower. That's OK, though - what we care about for this test
  // is just that the operations Apply() in the same order everywhere (even
  // though in this case the result will just be an error).
  WriteRequestPB req;
...
void RaftConsensusITest::StopOrKillLeaderAndElectNewOne() {
  ...
  ExternalTabletServer* leader_ets =
  ...
  // When all are paused also pause or kill the current leader. Since we've
  // waited a bit the old leader is likely to have operations that must be
  // aborted.
...
void RaftConsensusITest::Write128KOpsToLeader(int num_writes) {
  ...
  for (int i = 0; i < num_writes; i++) {
    ...
    data->Clear();
    AddTestRowToPB(
        RowOperationsPB::INSERT, schema_, key, key, test_payload, data);
    key++;
...
void RaftConsensusITest::AssertMajorityRequiredForElectionsAndWrites(
  ...
  // This math is a little unintuitive but works for cluster sizes including 2
  // and 1. Note: We assume all of these TSes are voters.
  ...
  if (config_size > 1) {
    ...
    LOG(INFO) << "Pausing " << num_to_pause
              << " tablet servers in config of size " << config_size;
    ...
    // Step down.
    ASSERT_OK(
        LeaderStepDown(initial_leader, tablet_id_, MonoDelta::FromSeconds(10)));

    // Assert that elections time out without a live majority.
    // We specify a very short timeout here to keep the tests fast.
    ASSERT_OK(
        StartElection(initial_leader, tablet_id_, MonoDelta::FromSeconds(10)));
    s = WaitUntilLeader(
        initial_leader, tablet_id_, MonoDelta::FromMilliseconds(100));
    ASSERT_TRUE(s.IsTimedOut()) << s.ToString();
    LOG(INFO)
        << "Expected timeout encountered on election with weakened config: "
        << s.ToString();

    // Resume the paused servers.
    LOG(INFO) << "Resuming " << num_to_pause
              << " tablet servers in config of size " << config_size;
  ...
  ASSERT_OK(WaitForServersToAgree(
      MonoDelta::FromSeconds(20), tablet_servers, tablet_id_, 1));

  // Now an election should succeed.
  ASSERT_OK(
      StartElection(initial_leader, tablet_id_, MonoDelta::FromSeconds(10)));
  ASSERT_OK(
      WaitUntilLeader(initial_leader, tablet_id_, MonoDelta::FromSeconds(10)));
...
void RaftConsensusITest::DoTestCrashyNodes(
  ...
  // failures we can verify an exact number of rows in the end, thanks to
  ...
  for (int i = 0; i < cluster_->num_tablet_servers(); i++) {
    ...
    ASSERT_TRUE(removed_flag)
        << "could not remove flag from TS " << i << "\nFlags:\n"
        << *flags;
    ts->Shutdown();
...
void RaftConsensusITest::SetupSingleReplicaTest(TServerDetails** replica_ts) {
  ...
  // Elect server 2 as leader and wait for log index 1 to propagate to all
  // servers.
  ASSERT_OK(StartElection(tservers[2], tablet_id_, MonoDelta::FromSeconds(10)));
  ASSERT_OK(WaitForServersToAgree(
      MonoDelta::FromSeconds(10), tablet_servers_, tablet_id_, 1));
...
TEST_F(RaftConsensusITest, TestGetPermanentUuid) {
  ...
  TServerDetails* leader = nullptr;
  ASSERT_OK(GetLeaderReplicaWithRetries(tablet_id_, &leader));
  peer.mutable_last_known_addr()->CopyFrom(
      leader->registration.rpc_addresses(0));
...
TEST_F(RaftConsensusITest, TestFailedTransaction) {
  ...
  ASSERT_OK(DCHECK_NOTNULL(leader->tserver_proxy.get())
                ->Write(req, &resp, &controller));
  ASSERT_TRUE(resp.has_error());

  // Add a proper row so that we can verify that all of the replicas continue
  // to process transactions after a failure. Additionally, this allows us to
  // wait for all of the replicas to finish processing transactions before
  // shutting down, avoiding a potential stall as we currently can't abort
  // transactions (see KUDU-341).
  data->Clear();
...
TEST_F(RaftConsensusITest, MultiThreadedMutateAndInsertThroughConsensus) {
  ...
  for (int i = 0; i < FLAGS_num_replicas; i++) {
    ...
    CHECK_OK(kudu::Thread::Create(
        "test",
        Substitute("chaos-test$0", i),
        &RaftConsensusITest::DelayInjectorThread,
        this,
        cluster_->tablet_server(i),
  ...
  NO_FATALS(AssertAllReplicasAgree(
      FLAGS_client_inserts_per_thread * FLAGS_num_client_threads));
}
...
TEST_F(RaftConsensusITest, TestInsertOnNonLeader) {
  ...
  ASSERT_STR_CONTAINS(
      s.ToString(), "is not leader of this config. Role: FOLLOWER");
  // TODO(unknown): need to change the error code to be something like
  // REPLICA_NOT_LEADER so that the client can properly handle this case!
  // plumbing this is a little difficult so not addressing at the moment.
...
TEST_F(RaftConsensusITest, TestCatchupAfterOpsEvicted) {
  const vector<string> kTsFlags = {
      ...
      // And disable WAL compression so the 128KB cells don't get compressed
      // away.
  ...
  // Now unpause the replica, the lagging replica should eventually catch back
  // up.
  ASSERT_OK(replica_ets->Resume());
  NO_FATALS(AssertAllReplicasAgree(kNumWrites));

  // Once the follower has caught up, all replicas should eventually GC the
  // earlier log segments that they were retaining.
  ASSERT_EVENTUALLY([&]() {
    for (int i = 0; i < cluster_->num_tablet_servers(); i++) {
...
TEST_F(RaftConsensusITest, TestFollowerFallsBehindLeaderGC) {
  ...
  SCOPED_TRACE(
      Substitute("leader: $0 follower: $1", leader_uuid, follower_uuid));
  ...
  if (AllowSlowTests()) {
    ...
    // This ensures that the other servers properly reject the pre-election
    // requests from the abandoned node, and that the abandoned node doesn't
    // bump its term either, since that would cause spurious leader elections
    // upon the node coming back to life.
    ...
    ASSERT_EQ(orig_term, op_id.term())
        << "expected the leader to have not advanced terms but has op "
        << op_id;
  }
}
...
// This emulates cases where there are many duplicate keys which, due to two
// phase locking, may cause deadlocks and other anomalies that cannot be
// observed when keys are unique.
...
TEST_F(RaftConsensusITest, MultiThreadedInsertWithFailovers) {
  ...
  // Reset consensus rpc timeout to the default value or the election might fail
  // often.
  FLAGS_consensus_rpc_timeout_ms = 1000;

  // Start a 7 node configuration cluster (since we can't bring leaders back we
  // start with a higher replica count so that we kill more leaders).
  ...
  // We create 2 * (kNumReplicas - 1) latches so that we kill the same node at
  // least twice.
...
// In this scenario, it would incorrectly sequence the client request's PREPARE
// phase before the operations received in step (3), even though the correct
// behavior would be to either reject them or sequence them after those
// operations, because the operation index is higher.
//
// The test works by setting up three replicas and manually hammering them with
// write requests targeting a single row. If the bug exists, then
// TransactionOrderVerifier will trigger an assertion because the prepare order
// and the op indexes will become misaligned.
...
TEST_F(RaftConsensusITest, TestLMPMismatchOnRestartedReplica) {
  ...
  ConsensusServiceProxy* c_proxy =
      CHECK_NOTNULL(replica_ts->consensus_proxy.get());
  ConsensusRequestPB req;
  ...
  ASSERT_EVENTUALLY([&]() {
    ...
    OpId commit;
    ASSERT_OK(
        lv.ScanForHighestCommittedOpIdInLog(replica_idx, tablet_id_, &commit));
  ...
  // Even though the replica previously received operations through 2.3, the LMP
  // mismatch above causes us to truncate operation 2.3, so 2.2 remains.
...
TEST_F(RaftConsensusITest, TestReplaceOperationStuckInPrepareQueue) {
  ...
  int64_t base_ts = GetTimestampOnServer(replica_ts);
  AddOpWithTypeAndKey(
      MakeOpId(2, 2), base_ts, RowOperationsPB::UPSERT, 1, &req);
  AddOpWithTypeAndKey(
      MakeOpId(2, 3), base_ts, RowOperationsPB::UPSERT, 1, &req);
  AddOpWithTypeAndKey(
      MakeOpId(2, 4), base_ts, RowOperationsPB::UPSERT, 1, &req);
  req.set_committed_index(2);
  rpc.Reset();
  ASSERT_OK(c_proxy->UpdateConsensus(req, &resp, &rpc));
  ASSERT_FALSE(resp.has_error()) << SecureDebugString(resp);
  ...
  req.clear_ops();
  AddOpWithTypeAndKey(
      MakeOpId(3, 4), base_ts, RowOperationsPB::UPSERT, 1, &req);
  AddOpWithTypeAndKey(
      MakeOpId(3, 5), base_ts, RowOperationsPB::UPSERT, 2, &req);
  rpc.Reset();
  ...
  // We need to ASSERT_EVENTUALLY here because otherwise it's possible to read
  // the old value of row '1', if the operation is still in flight.
  ASSERT_EVENTUALLY([&]() {
    vector<string> results;
    NO_FATALS(WaitForRowCount(replica_ts->tserver_proxy.get(), 2, &results));
    ASSERT_EQ(
        "(int32 key=1, int32 int_val=3, string string_val=\"term: 3 index: 4\")",
        results[0]);
    ASSERT_EQ(
        "(int32 key=2, int32 int_val=3, string string_val=\"term: 3 index: 5\")",
        results[1]);
  });
}
...
TEST_F(RaftConsensusITest, TestReplicaBehaviorViaRPC) {
  ...
  {
    int64_t term_from_metric = -1;
    ASSERT_OK(GetTermMetricValue(
        cluster_->tablet_server_by_uuid(replica_ts->uuid()),
        &term_from_metric));
  ...
  // hanging the RPC service thread.
  {
    ...
    // Send the call. We expect to get a timeout passed back from the server
    // side (i.e. not an RPC timeout)
  ...
  // Verify the new rows are committed.
  {
    ...
    ASSERT_STR_CONTAINS(
        results[3], Substitute("term: $0 index: 5", leader_term));
    ASSERT_STR_CONTAINS(
        results[4], Substitute("term: $0 index: 6", leader_term));
  }
}
...
TEST_F(RaftConsensusITest, TestAddRemoveServer) {
  ...
  ASSERT_OK(
      WaitUntilCommittedOpIdIndexIs(1, leader_tserver, tablet_id_, kTimeout));
  ...
  NO_FATALS(AssertMajorityRequiredForElectionsAndWrites(
      active_tablet_servers, leader_uuid));
  OpId opid;
  ASSERT_OK(GetLastOpIdForReplica(
      tablet_id_, leader_tserver, consensus::RECEIVED_OPID, kTimeout, &opid));
  ...
  for (int to_remove_idx : remove_list) {
    ...
    LOG(INFO) << "Remove: Going from " << num_servers << " to "
              << num_servers - 1 << " replicas";

    TServerDetails* tserver_to_remove = tservers[to_remove_idx];
    LOG(INFO) << "Removing tserver with uuid " << tserver_to_remove->uuid();
    ASSERT_OK(
        RemoveServer(leader_tserver, tablet_id_, tserver_to_remove, kTimeout));
    ASSERT_EQ(1, active_tablet_servers.erase(tserver_to_remove->uuid()));
    ASSERT_OK(WaitForServersToAgree(
        kTimeout, active_tablet_servers, tablet_id_, ++cur_log_index));
  ...
  // Add the tablet servers back, in reverse order, going from 1 to 3 servers in
  // the configuration.
  vector<int> add_list = {1, 2};
  for (int to_add_idx : add_list) {
    int num_servers = active_tablet_servers.size();
    LOG(INFO) << "Add: Going from " << num_servers << " to " << num_servers + 1
              << " replicas";
...
// Regression test for KUDU-1169: a crash when a Config Change operation is
...
TEST_F(RaftConsensusITest, TestReplaceChangeConfigOperation) {
  ...
  ASSERT_OK(
      StartElection(leader_tserver, tablet_id_, MonoDelta::FromSeconds(10)));
  ASSERT_OK(WaitForServersToAgree(
      MonoDelta::FromSeconds(10), tablet_servers_, tablet_id_, 1));
  ...
  Status s = RemoveServer(
      leader_tserver, tablet_id_, tservers[1], MonoDelta::FromSeconds(1), -1);
  ASSERT_TRUE(s.IsTimedOut()) << s.ToString();
  ...
  ASSERT_OK(WaitForServersToAgree(
      MonoDelta::FromSeconds(10), original_followers, tablet_id_, 1));

  // Elect one of the other servers.
  ASSERT_OK(StartElection(tservers[1], tablet_id_, MonoDelta::FromSeconds(10)));
  ASSERT_OK(
      WaitUntilLeader(tservers[1], tablet_id_, MonoDelta::FromSeconds(10)));
  ...
  // This acts as a regression test for KUDU-1338, in which aborting the
  // original config change didn't properly unset the 'pending' configuration.
  ASSERT_OK(RemoveServer(
      leader_tserver, tablet_id_, tservers[2], MonoDelta::FromSeconds(5), -1));
...
TEST_F(RaftConsensusITest, TestAtomicAddRemoveServer) {
  ...
  ASSERT_OK(WaitUntilCommittedOpIdIndexIs(
      1, leader_tserver, tablet_id_, MonoDelta::FromSeconds(10)));
  ...
  ASSERT_STR_CONTAINS(
      s.ToString(), "of 7 but the committed config has opid_index of -1");
  ...
  ASSERT_EQ(TabletServerErrorPB::CAS_FAILED, error_code);
  ASSERT_STR_CONTAINS(
      s.ToString(), "of -1 but the committed config has opid_index of 2");
...
static void DoWriteTestRows(
    const TServerDetails* leader_tserver,
  ...
  while (!finish->Load()) {
    ...
    CHECK_OK(WriteSimpleTestRow(
        leader_tserver,
        tablet_id,
        RowOperationsPB::INSERT,
        row_key,
        row_key,
        Substitute("key=$0", row_key),
        write_timeout));
  }
}
...
TEST_F(RaftConsensusITest, TestConfigChangeUnderLoad) {
  ...
  ASSERT_OK(WaitForOpFromCurrentTerm(
      leader_tserver, tablet_id_, consensus::COMMITTED_OPID, kTimeout));
  ...
  for (int to_remove_idx : remove_list) {
    ...
    LOG(INFO) << "Removing tserver with uuid " << tserver_to_remove->uuid();
    ASSERT_OK(
        WaitForLeaderWithCommittedOp(tablet_id_, kTimeout, &leader_tserver));
    ASSERT_OK(
        RemoveServer(leader_tserver, tablet_id_, tserver_to_remove, kTimeout));
    ASSERT_EQ(1, active_tablet_servers.erase(tserver_to_remove->uuid()));
    ASSERT_OK(WaitUntilCommittedConfigNumVotersIs(
        active_tablet_servers.size(), leader_tserver, tablet_id_, kTimeout));
  }
...
TEST_F(RaftConsensusITest, TestMasterNotifiedOnConfigChange) {
  ...
  const vector<string> kMasterFlags = {
      ...
      // behavior since we want the newly added replica to stay.
  ...
  for (itest::TabletReplicaMap::const_iterator iter =
           tablet_replicas_.find(tablet_id);
       iter != tablet_replicas_.end();
       ++iter) {
  ...
  LOG(INFO) << "Adding tserver with uuid " << tserver_to_add->uuid();
  ASSERT_OK(AddServer(
      leader_ts, tablet_id_, tserver_to_add, RaftPeerPB::VOTER, timeout));
  ...
  ASSERT_OK(
      WaitForServersToAgree(timeout, active_tablet_servers, tablet_id_, 3));
...
TEST_F(RaftConsensusITest, TestEarlyCommitDespiteMemoryPressure) {
  ...
  const vector<string> kTsFlags = {
      ...
      // When using tcmalloc, we set it to 30MB, since we can get accurate
      // process memory
      // usage statistics. Otherwise, set to only 4MB, since we'll only be
      // throttling based
      // on our tracked memory.
      // Since part of the point of the test is to have memory pressure, don't
      // insist the block cache capacity is small compared to the memory
      // pressure
      // threshold.
...
TEST_F(RaftConsensusITest, TestAutoCreateReplica) {
  ...
  while (true) {
    ...
    LOG(INFO) << "Only inserted " << rows_inserted
              << " rows so far, sleeping for 100ms";
    SleepFor(MonoDelta::FromMilliseconds(100));
  }
...
TEST_F(RaftConsensusITest, TestLargeBatches) {
  const vector<string> kTsFlags = {
      // We write 128KB cells in this test, so bump the limit, and disable
      // compression.
      "--max_cell_size_bytes=1000000",
      "--log_segment_size_mb=1",
      "--log_compression_codec=none",
...
// Regression test for KUDU-1469, a case in which a leader and follower could
// get "stuck" in a tight RPC loop, in which the leader would repeatedly send a
// batch of ops that the follower already had, the follower would fully de-dupe
// them, and yet the leader would never advance to the next batch.
//
...
// - before the old leader notices it has been ousted, it writes at least one
// more
...
TEST_F(RaftConsensusITest, TestCommitIndexFarBehindAfterLeaderElection) {
  ...
  TServerDetails* leader;
  ASSERT_OK(GetLeaderReplicaWithRetries(tablet_id_, &leader));
  ExternalTabletServer* first_leader_ts =
      cluster_->tablet_server_by_uuid(leader->uuid());
  ...
  ASSERT_OK(cluster_->SetFlag(
      only_vote_ts, "follower_reject_update_consensus_requests", "true"));

  // Inject a long delay in the log of the first leader, and write 10
  // operations. This delay ensures that it will replicate them to both itself
  // and its follower, but due to its log sync not completing, it won't know
  // that it is safe to advance its commit index until long after it has lost
  // its leadership.
  ASSERT_OK(
      cluster_->SetFlag(first_leader_ts, "log_inject_latency_ms_mean", "6000"));
  ASSERT_OK(cluster_->SetFlag(first_leader_ts, "log_inject_latency", "true"));
  InsertPayloadIgnoreErrors(0, 10, 10000);

  // Write one more operation to the leader, but disable consensus on the
  // follower so that it doesn't get replicated.
  ASSERT_OK(cluster_->SetFlag(
  ...
  // third TS participates here by voting.
  ASSERT_OK(first_leader_ts->Pause());
  ASSERT_OK(WaitUntilLeader(
      tablet_servers_[second_leader_ts->uuid()], tablet_id_, kTimeout));

  // The voter TS has done its duty. Shut it down to avoid log spam where it
  // tries to run elections.
  only_vote_ts->Shutdown();

  // Perform one insert on the new leader. The new leader has not yet replicated
  // its NO_OP to the old leader, since the old leader is still paused.
...
TEST_F(RaftConsensusITest, TestSlowFollower) {
  ...
  for (int i = 0; i < cluster_->num_tablet_servers(); i++) {
    ...
    if (ts->instance_id().permanent_uuid() != leader->uuid()) {
      ...
      follower = GetReplicaWithUuidOrNull(
...
TEST_F(RaftConsensusITest, TestEvictAbandonedFollowers) {
  ...
  ASSERT_OK(WaitUntilCommittedConfigNumVotersIs(
      2, tablet_servers_[leader_uuid], tablet_id_, timeout));
  ASSERT_EQ(1, active_tablet_servers.erase(follower_uuid));
  ASSERT_OK(
      WaitForServersToAgree(timeout, active_tablet_servers, tablet_id_, 2));
}

// Test that, after followers are evicted from the config, the master re-adds a
// new replica for that follower and it eventually catches back up.
TEST_F(RaftConsensusITest, TestMasterReplacesEvictedFollowers) {
  ...
  if (FLAGS_raft_prepare_replacement_before_eviction) {
    ...
    // copied.
    ASSERT_OK(
        WaitForServersToAgree(kTimeout, active_tablet_servers, tablet_id_, 2));
  }
...
TEST_F(RaftConsensusITest, TestChangeConfigRejectedUnlessNoopReplicated) {
  ...
  int kLeaderIndex = 0;
  TServerDetails* leader_ts =
      tablet_servers_[cluster_->tablet_server(kLeaderIndex)->uuid()];
...
TEST_P(RaftConsensusParamReplicationModesITest, Test_KUDU_1735) {
  ...
  ASSERT_OK(
      RemoveServer(leader_tserver, tablet_id_, evicted_tserver, kTimeout));
  for (const auto& e : tablet_servers_) {
    const auto& server_uuid = e.first;
    if (server_uuid == leader_tserver->uuid()) {
  ...
  // Delete the table, so that when we restart the crashed servers, they'll get
  // RPCs to delete tablets while config changes are pending.
...
// Test that if for some reason none of the transactions can be prepared, that
// it will come back as an error in UpdateConsensus().
...
TEST_F(RaftConsensusITest, TestCorruptReplicaMetadata) {
  ...
  // Switch off tombstoning of evicted replicas to observe the failed tablet
  // state.
  ...
  ASSERT_OK(WaitForServersToAgree(
      MonoDelta::FromSeconds(10), tablet_servers, tablet_id_, 1));

  // Shut down one of the tablet servers, and then muck
  // with its consensus metadata to corrupt it.
  const auto ts_idx =
      cluster_->tablet_server_index_by_uuid(uuids_with_replica.front());
  ...
  ASSERT_OK(WaitUntilTabletInState(
      tablet_servers_[ts->uuid()], tablet_id_, tablet::FAILED, kTimeout));
  // A new good copy should get created automatically.
  ASSERT_OK(WaitUntilTabletInState(
      tablet_servers_[uuids_no_replica.front()],
      tablet_id_,
      tablet::RUNNING,
      kTimeout));
}

int64_t GetFailedElectionsSinceStableLeader(
    const ExternalTabletServer* ets,
...
int64_t GetTimeSinceLastLeaderHeartbeat(
    const ExternalTabletServer* ets,
    const std::string& tablet_id) {
  int64_t ret;
  CHECK_OK(GetInt64Metric(
...
TEST_F(RaftConsensusITest, TestElectionMetricsFailureDetectionDisabled) {
  ...
  const int64_t time_before_wait =
      GetTimeSinceLastLeaderHeartbeat(follower_srv, tablet_id_);
  SleepFor(MonoDelta::FromMilliseconds(500));
  const int64_t time_after_wait =
      GetTimeSinceLastLeaderHeartbeat(follower_srv, tablet_id_);
...
TEST_F(RaftConsensusITest, TestElectionMetricsFailureDetectionEnabled) {
  ...
  const vector<string> kTsFlags = {
      ...
      // of on-going re-elections the metric value would not stabilize for a
      // long
      // time.
  ...
  ASSERT_EVENTUALLY([&]() {
    for (auto i = 0; i < kNumTservers; ++i) {
      ASSERT_EQ(
          0,
          GetFailedElectionsSinceStableLeader(
...
TEST_F(RaftConsensusITest, TestLogIOErrorIsFatal) {
  ...
  for (int i = 1; i <= 2; i++) {
    ASSERT_OK(cluster_->SetFlag(
        ext_tservers[i], "log_inject_io_error_on_append_fraction", "1.0"));
  }
  ...
  ASSERT_OK(cluster_->SetFlag(
      ext_tservers[0], "log_inject_io_error_on_append_fraction", "1.0"));
...
TEST_P(RaftConsensusParamReplicationModesITest, TestRestartWithDifferentUUID) {
  ...
  LOG(INFO) << Substitute(
      "Storing port $0 for use by new tablet server",
  ...
  ASSERT_EVENTUALLY([&] {
    ...
    ASSERT_OK(ListFilesInDir(
        env_, JoinPathSegments(ts_opts.wal_dir, "wals"), &files_in_wal_dir));
