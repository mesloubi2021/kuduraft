...
// This integration test tries to trigger all the update-related bits while also
// serving as a foundation for benchmarking. It first inserts 'row_count' rows
// and then starts two threads, one that continuously updates all the rows
// sequentially and one that scans them all, until it's been running for
// 'seconds_to_run'. It doesn't test for correctness, unless something FATALs.
class UpdateScanDeltaCompactionTest : public KuduTest {
 ...
 private:
  enum { kKeyCol, kStrCol, kInt64Col };
  ...
  // If 'key' is a multiple of kSessionBatchSize, it uses 'last_s' to wait for
  // the previous batch to finish and then flushes the current one.
  Status WaitForLastBatchAndFlush(
      int64_t key,
      Synchronizer* last_s,
...
const char* const UpdateScanDeltaCompactionTest::kTableName =
    "update-scan-delta-compact-tbl";
...
TEST_F(UpdateScanDeltaCompactionTest, TestAll) {
  ...
  // Setting this high enough that we see the effects of flushes and
  // compactions.
...
void UpdateScanDeltaCompactionTest::InsertBaseData() {
  ...
  Synchronizer last_s;
  KuduStatusMemberCallback<Synchronizer> last_s_cb(
      &last_s, &Synchronizer::StatusCB);
  ...
  LOG_TIMING(INFO, "Insert") {
    ...
    ASSERT_OK(WaitForLastBatchAndFlush(
        kSessionBatchSize, &last_s, &last_s_cb, session));
...
void UpdateScanDeltaCompactionTest::RunThreads() {
...

  {
    ...
    ASSERT_OK(Thread::Create(
        ...
        &UpdateScanDeltaCompactionTest::UpdateRows,
        this,
        &stop_latch,
        &t));
    threads.push_back(t);
...

  {
    ...
    ASSERT_OK(Thread::Create(
        ...
        &UpdateScanDeltaCompactionTest::ScanRows,
        this,
        &stop_latch,
        &t));
    threads.push_back(t);
...

  {
    ...
    ASSERT_OK(Thread::Create(
        ...
        &UpdateScanDeltaCompactionTest::CurlWebPages,
        this,
        &stop_latch,
        &t));
    threads.push_back(t);
...
void UpdateScanDeltaCompactionTest::UpdateRows(CountDownLatch* stop_latch) {
  ...
  for (int64_t iteration = 1; stop_latch->count() > 0; iteration++) {
    ...
    LOG_TIMING(INFO, "Update") {
      ...
      CHECK_OK(WaitForLastBatchAndFlush(
          kSessionBatchSize, &last_s, &last_s_cb, session));
...
void UpdateScanDeltaCompactionTest::CurlWebPages(
    CountDownLatch* stop_latch) const {
  vector<string> urls;
  string base_url =
...
void UpdateScanDeltaCompactionTest::MakeRow(
    int64_t key,
    int64_t val,
