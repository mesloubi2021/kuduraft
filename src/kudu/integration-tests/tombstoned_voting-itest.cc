...
TEST_F(TombstonedVotingITest, TestTombstonedReplicaWithoutCMetaCanVote) {
  ...
  NO_FATALS(StartCluster(
      {"--tablet_copy_fault_crash_before_write_cmeta=1.0"},
      {},
  ...
  workload.Start();
  ASSERT_EVENTUALLY([&] { ASSERT_GE(10, workload.batches_completed()); });
  ...
  ASSERT_EVENTUALLY([&] {
    ...
    Status s = itest::StartTabletCopy(
        new_replica_ts,
        tablet_id,
        leader_ts->uuid(),
  ...
  ASSERT_OK(inspect_->CheckTabletDataStateOnTS(
      new_replica_idx, tablet_id, {TABLET_DATA_COPYING}));
  ...
  ASSERT_OK(inspect_->ReadTabletSuperBlockOnTS(
      new_replica_idx, tablet_id, &superblock_pb));
  ...
  ASSERT_EVENTUALLY([&] {
    ...
    ASSERT_GE(
        heartbeat_count, 5); // Wait for 5 heartbeats from the only live TS.
  });
