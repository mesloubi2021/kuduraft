...
//
// If 'pad_unixtime_micros_to_16_bytes' is true, UNIXTIME_MICROS slots in the
// projection schema will be padded to the right by 8 (zero'd) bytes for a total
// of 16 bytes.
//
...
//
// It 'pad_unixtime_micros_to_16_bytes' is true, this function will take padding
// into account when rewriting the block pointers. See: SerializeRowBlock() for
// the actual format.
//
...
// 'indirect_data'. This is also the reason that 'rows_data' is a non-const
// pointer argument: the internal data is mutated in-place to restore the
// validity of indirect data pointers, which are relative on the wire but must
// be absolute while in-memory.
//
// Returns a bad Status if the provided data is invalid or corrupt.
...
Status FindLeaderHostPort(
    const google::protobuf::RepeatedPtrField<ServerEntryPB>& entries,
