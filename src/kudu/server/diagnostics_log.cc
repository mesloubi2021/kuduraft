...
DEFINE_bool(
    diagnostics_thread_refresh_histogram_stats,
    false,
  ...
  switch (type) {
    case WakeupType::STACKS:
      if (FLAGS_diagnostics_log_stack_traces_interval_ms > 0) {
        ...
        // biasing our stack samples. For example, if there is some background
        // process which happens once a minute, and the user also configured the
        ...
        // because the flag is runtime-modifiable, and we need to wake up to
...
Status DiagnosticsLog::LogMetrics() {
  ...
  opts.refresh_histogram_metrics =
      FLAGS_diagnostics_thread_refresh_histogram_stats;
