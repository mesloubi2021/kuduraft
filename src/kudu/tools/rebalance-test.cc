...
bool operator==(const TableBalanceInfo& lhs, const TableBalanceInfo& rhs) {
  return HasSameContents(
      lhs.servers_by_replica_count, rhs.servers_by_replica_count);
}

bool operator==(const ClusterBalanceInfo& lhs, const ClusterBalanceInfo& rhs) {
  return lhs.table_info_by_skew == rhs.table_info_by_skew &&
      HasSameContents(
             lhs.servers_by_total_replica_count,
...
class KsckResultsToClusterBalanceInfoTest : public ::testing::Test {
 protected:
  void RunTest(
      const Rebalancer::Config& rebalancer_cfg,
