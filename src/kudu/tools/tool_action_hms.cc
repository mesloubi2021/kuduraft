...
Status RenameTableInKuduCatalog(
    KuduClient* kudu_client,
    const string& name,
    const string& new_name) {
  unique_ptr<KuduTableAlterer> alterer(kudu_client->NewTableAlterer(name));
  return alterer->RenameTo(new_name)->modify_external_catalogs(false)->Alter();
}
...
Status Init(
    ...
    string* master_addrs) {
  const string& master_addrs_flag =
...
// Prints catalog information about Kudu and HMS tables in data table format to
// 'out'.
Status PrintTables(
    const string& master_addrs,
...
struct CatalogReport {
  ...
  // Kudu tables with multiple HMS table entries. The entries may or may not be
  // legacy.
  //
  // These tables can not be automatically corrected by the fix tool.
  ...
  bool empty() const {
    return orphan_hms_tables.empty() && legacy_hms_tables.empty() &&
        duplicate_hms_tables.empty() && inconsistent_tables.empty() &&
        missing_hms_tables.empty() && invalid_name_tables.empty();
  }
};
...
Status AnalyzeCatalogs(
  ...
  unordered_map<string, shared_ptr<KuduTable>> kudu_tables_by_name;
  {
    ...
    for (const string& kudu_table_name : kudu_table_names) {
      ...
      // not running this tool when the catalog is in-flux.
  ...
  unordered_map<string, vector<hive::Table>> hms_tables_by_id;
  {
    ...
    for (hive::Table& hms_table : hms_tables) {
      ...
      if (storage_handler == HmsClient::kKuduStorageHandler) {
        ...
        shared_ptr<KuduTable>* kudu_table =
            FindOrNull(kudu_tables_by_id, hms_table_id);
        if (kudu_table) {
          hms_tables_by_id[(*kudu_table)->id()].emplace_back(
              std::move(hms_table));
        } else {
        ...
        const string& hms_table_name =
            hms_table.parameters[HmsClient::kLegacyKuduTableNameKey];
        shared_ptr<KuduTable>* kudu_table =
            FindOrNull(kudu_tables_by_name, hms_table_name);
        if (kudu_table) {
          hms_tables_by_id[(*kudu_table)->id()].emplace_back(
              std::move(hms_table));
        } else {
  ...
  for (auto& kudu_table_pair : kudu_tables_by_id) {
    ...
    vector<hive::Table>* hms_tables =
        FindOrNull(hms_tables_by_id, kudu_table_pair.first);
    ...
    if (!hms_tables) {
      ...
      if (storage_handler == HmsClient::kKuduStorageHandler &&
        ...
        stale_tables.emplace_back(
            make_pair(std::move(kudu_table), std::move(hms_table)));
      } else if (storage_handler == HmsClient::kLegacyKuduStorageHandler) {
        legacy_tables.emplace_back(
            make_pair(std::move(kudu_table), std::move(hms_table)));
      }
    } else {
      for (hive::Table& hms_table : *hms_tables) {
        duplicate_tables.emplace_back(
            make_pair(kudu_table, std::move(hms_table)));
      }
    }
  }
...
Status CheckHmsMetadata(const RunnerContext& context) {
  ...
  // (--hive_metastore_uris, --hive_metastore_sasl_enabled) match on all
  // masters.
  shared_ptr<KuduClient> kudu_client;
  unique_ptr<HmsCatalog> hms_catalog;
  string master_addrs;
  ...
  CatalogReport report;
  RETURN_NOT_OK(AnalyzeCatalogs(
      master_addrs, hms_catalog.get(), kudu_client.get(), &report));
  ...
  if (!report.invalid_name_tables.empty()) {
    ...
    RETURN_NOT_OK(
        PrintKuduTables(master_addrs, report.invalid_name_tables, cout));
    cout
        << endl
  ...
  if (!report.duplicate_hms_tables.empty()) {
    ...
    cout
        << "Found Kudu table(s) with multiple corresponding Hive Metastore tables:"
        << endl;
  ...
  if (!report.orphan_hms_tables.empty() || !report.missing_hms_tables.empty() ||
    ...
    cout
        << "Found table(s) with missing or inconsisentent metadata in the Kudu "
           "catalog or Hive Metastore:"
        << endl;
    RETURN_NOT_OK(PrintTables(master_addrs, std::move(tables), cout));
    ...
    if (report.orphan_hms_tables.empty()) {
      ...
      cout
          ...
          << "Suggestion: use the fix tool to repair the Kudu and Hive Metastore catalog metadata"
          << endl
          << "and drop Hive Metastore table(s) which reference non-existent Kudu tables:"
          << endl
  ...
  return Status::IllegalState(
      "found inconsistencies in the Kudu and HMS catalogs");
}
...
Status FixHmsMetadata(const RunnerContext& context) {
  ...
  if (FLAGS_drop_orphan_hms_tables) {
    for (hive::Table& hms_table : report.orphan_hms_tables) {
      ...
      if (FLAGS_dryrun) {
        ...
        const string& storage_handler =
            hms_table.parameters[HmsClient::kStorageHandlerKey];
        // All errors are fatal here, since we've already checked that the table
        // exists in the HMS.
        if (storage_handler == HmsClient::kKuduStorageHandler) {
          RETURN_NOT_OK_PREPEND(
              hms_catalog->DropTable(table_id, table_name),
              Substitute("failed to drop orphan HMS table $0", table_name));
        } else {
          RETURN_NOT_OK_PREPEND(
              hms_catalog->DropLegacyTable(table_name),
              Substitute(
                  "failed to drop legacy orphan HMS table $0", table_name));
        }
      }
    }
  }
    ...
    for (const auto& kudu_table : report.missing_hms_tables) {
      ...
      if (FLAGS_dryrun) {
        ...
        RETURN_NOT_OK_PREPEND(
            s,
            Substitute(
                "failed to create HMS table for Kudu table $0",
                TableIdent(*kudu_table)));
      }
      ...
      if (normalized_table_name != table_name) {
        if (FLAGS_dryrun) {
          ...
          RETURN_NOT_OK_PREPEND(
              RenameTableInKuduCatalog(
                  kudu_client.get(), table_name, normalized_table_name),
              Substitute(
                  "failed to rename Kudu table $0 to lowercased Hive compatible name $1",
                  TableIdent(*kudu_table),
                  normalized_table_name));
        }
      }
  ...
  if (FLAGS_upgrade_hms_tables) {
    for (const auto& table_pair : report.legacy_hms_tables) {
      ...
      if (FLAGS_dryrun) {
        ...
        RETURN_NOT_OK_PREPEND(
            hms_catalog->UpgradeLegacyImpalaTable(
                kudu_table.id(),
                hms_table.dbName,
                hms_table.tableName,
                client::SchemaFromKuduSchema(kudu_table.schema())),
            Substitute(
                "failed to upgrade legacy Impala HMS metadata for table $0",
                hms_table_name));
      }
      ...
      if (kudu_table.name() != hms_table_name) {
        if (FLAGS_dryrun) {
          ...
          Status s = RenameTableInKuduCatalog(
              kudu_client.get(), kudu_table.name(), hms_table_name);
          if (s.IsAlreadyPresent()) {
            LOG(ERROR) << "Failed to rename Kudu table "
                       ...
                       << " already exists";
            LOG(INFO)
                << "Suggestion: rename the conflicting table name manually:\n"
          ...
          RETURN_NOT_OK_PREPEND(
              s,
              Substitute(
                  "failed to rename Kudu table $0 to $1",
                  TableIdent(kudu_table),
                  hms_table_name));
        }
      }
    ...
    for (const auto& table_pair : report.inconsistent_tables) {
      ...
      if (hms_table_name != kudu_table.name()) {
        ...
        if (FLAGS_dryrun) {
          ...
          if (s.IsAlreadyPresent()) {
            LOG(ERROR) << "Failed to rename Kudu table "
                       << TableIdent(kudu_table) << " to match HMS table "
                       << hms_table_name << ", because a Kudu table with name "
                       << hms_table_name << " already exists";
            success = false;
            continue;
          }
          RETURN_NOT_OK_PREPEND(
      ...
      if (FLAGS_dryrun) {
        ...
        Schema schema(client::SchemaFromKuduSchema(kudu_table.schema()));
        RETURN_NOT_OK_PREPEND(
            hms_catalog->AlterTable(
                kudu_table.id(), hms_table_name, hms_table_name, schema),
            Substitute(
                "failed to refresh HMS table metadata for Kudu table $0",
                TableIdent(kudu_table)));
      }
    }
  }
  ...
  return Status::RuntimeError(
      "Failed to fix some catalog metadata inconsistencies");
}
...
Status Precheck(const RunnerContext& context) {
  ...
  cout
      << "Found Kudu tables whose names are not unique according to Hive's case-insensitive"
      << endl
      << "identifier requirements. These conflicting tables will cause master startup to"
      << endl
      << "fail when the Hive Metastore integration is enabled:";
  RETURN_NOT_OK(data_table.PrintTo(cout));
  cout
      << endl
      << "Suggestion: rename the conflicting tables to case-insensitive unique names:"
      << endl
      << "\t$ kudu table rename_table " << master_addrs
      << " <conflicting_table_name> <new_table_name>" << endl;

  return Status::IllegalState(
      "found tables in Kudu with case-conflicting names");
}
...
unique_ptr<Mode> BuildHmsMode() {
  ...
  unique_ptr<Action> hms_fix =
      ActionBuilder("fix", &FixHmsMetadata)
          .Description(
              "Fix automatically-repairable metadata inconsistencies in the "
  ...
  unique_ptr<Action> hms_precheck =
      ActionBuilder("precheck", &Precheck)
          .Description(
              "Check that the Kudu cluster is prepared to enable the Hive "
              "Metastore integration")
          .AddRequiredParameter({kMasterAddressesArg, kMasterAddressesArgDesc})
          .Build();

  return ModeBuilder("hms")
      .Description("Operate on remote Hive Metastores")
