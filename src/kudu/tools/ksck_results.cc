...
// single-character labels using the mapping 'label_by_uuid'.
string format_replicas(
    const map<string, char>& label_by_uuid,
    const KsckConsensusState& config) {
...
void AddToUuidLabelMapping(
    const std::set<string>& uuids,
...
void AddRowForCState(
  ...
  const string opid_index_str =
      cstate.opid_index ? std::to_string(*cstate.opid_index) : "";
  const string term = cstate.term ? std::to_string(*cstate.term) : "";
  const string committed =
      cstate.type == KsckConsensusConfigType::PENDING ? "No" : "Yes";
...
bool ServerByHealthComparator(
    const KsckServerHealthSummary& left,
    const KsckServerHealthSummary& right) {
  return std::make_tuple(
             ServerHealthScore(left.health), left.uuid, left.address) <
      std::make_tuple(
             ServerHealthScore(right.health), right.uuid, right.address);
}
...
Status KsckResults::PrintTo(PrintMode mode, ostream& out) {
  ...
  std::sort(
      master_summaries.begin(),
      master_summaries.end(),
      ServerByHealthComparator);
  RETURN_NOT_OK(PrintServerHealthSummaries(
  ...
  RETURN_NOT_OK(PrintFlagTable(
      KsckServerType::MASTER,
  ...
  std::sort(
      tserver_summaries.begin(),
      tserver_summaries.end(),
      ServerByHealthComparator);
  RETURN_NOT_OK(PrintServerHealthSummaries(
      KsckServerType::TABLET_SERVER, tserver_summaries, out));
  if (!tserver_summaries.empty()) {
    out << endl;
  }

  RETURN_NOT_OK(PrintFlagTable(
      KsckServerType::TABLET_SERVER,
  ...
  std::sort(
      table_summaries.begin(),
      table_summaries.end(),
      [](const KsckTableSummary& left, const KsckTableSummary& right) {
        return std::make_pair(
                   left.TableStatus() != KsckCheckResult::HEALTHY, left.name) <
            std::make_pair(
                   right.TableStatus() != KsckCheckResult::HEALTHY, right.name);
      });
  ...
  // Remember, we only return non-OK if printing failed, not if ksck checks
  // failed.
  return Status::OK();
}
...
Status PrintServerHealthSummaries(
  ...
  // This isn't done as part of the table because the messages can be quite
  // long.
  for (const auto& s : summaries) {
...
Status PrintVersionTable(
    const vector<KsckServerHealthSummary>& masters,
...
Status PrintTableSummaries(
    const vector<KsckTableSummary>& table_summaries,
    ostream& out) {
  if (table_summaries.empty()) {
  ...
  for (const KsckTableSummary& ts : table_summaries) {
    table.AddRow(
        {ts.name,
         ...
         KsckCheckResultToString(ts.TableStatus()),
         to_string(ts.TotalTablets()),
         to_string(ts.healthy_tablets),
         to_string(ts.recovering_tablets),
...
Status PrintTabletSummaries(
    const vector<KsckTabletSummary>& tablet_summaries,
    PrintMode mode,
    ostream& out) {
  ...
  for (const auto& tablet_summary : tablet_summaries) {
    ...
    for (const KsckReplicaSummary& r : tablet_summary.replicas) {
      const char* spec_str =
          r.is_leader ? " [LEADER]" : (!r.is_voter ? " [NONVOTER]" : "");
    ...
    RETURN_NOT_OK(PrintConsensusMatrix(
        ts_uuids, master_cstate, consensus_state_map, out));
    out << endl;
  }
  return Status::OK();
}
...
Status PrintChecksumResults(
  ...
  if (checksum_results.snapshot_timestamp) {
    out << "Using snapshot timestamp: " << *checksum_results.snapshot_timestamp
        << endl;
  }
    ...
    for (const auto& tablet_entry : table_checksums) {
      ...
      for (const auto& replica_entry : tablet_checksum.replica_checksums) {
        ...
        out << Substitute(
                   ...
                   tablet_id,
                   ts_uuid,
                   replica_checksum.ts_address,
                   status_str)
            << endl;
      }
      if (tablet_checksum.mismatch) {
        out << ">> Mismatch found in table " << table_name << " tablet "
            << tablet_id << endl;
      }
    }
  }
...
Status PrintTotalCounts(const KsckResults& results, std::ostream& out) {
  ...
  int num_replicas = std::accumulate(
      results.tablet_summaries.begin(),
  ...
  totals.AddRow(
      {"Tablet Servers", to_string(results.tserver_summaries.size())});
...
void KsckServerHealthSummaryToPb(
    const KsckServerHealthSummary& summary,
...
void KsckReplicaSummaryToPb(
    const KsckReplicaSummary& replica,
  ...
  if (replica.consensus_state) {
    KsckConsensusStateToPb(
        *replica.consensus_state, pb->mutable_consensus_state());
  }
}
...
void KsckTabletSummaryToPb(
    const KsckTabletSummary& tablet,
...
void KsckTableSummaryToPb(
    const KsckTableSummary& table,
    KsckTableSummaryPB* pb) {
  pb->set_id(table.id);
...
void KsckTabletChecksumToPb(
  ...
  for (const auto& entry : tablet.replica_checksums) {
    KsckReplicaChecksumToPb(
        entry.first, entry.second, pb->add_replica_checksums());
  }
}

void KsckTableChecksumToPb(
    const string& name,
...
void KsckChecksumResultsToPb(
    const KsckChecksumResults& results,
