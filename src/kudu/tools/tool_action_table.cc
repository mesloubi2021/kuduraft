...
class TableLister {
 public:
  static Status ListTablets(const vector<string>& master_addresses) {
    ...
    RETURN_NOT_OK(builder.master_server_addrs(master_addresses).Build(&client));
    ...
    for (const auto& tname : table_names) {
      if (!MatchesAnyPattern(table_filters, tname))
        continue;
      cout << tname << endl;
...
Status DeleteTable(const RunnerContext& context) {
  ...
  return client->DeleteTableInCatalogs(
      table_name, FLAGS_modify_external_catalogs);
}
...
Status RenameColumn(const RunnerContext& context) {
  ...
  const string& new_column_name =
      FindOrDie(context.required_args, kNewColumnNameArg);
  ...
  unique_ptr<Action> rename_column =
      ActionBuilder("rename_column", &RenameColumn)
          ...
          .AddRequiredParameter(
              {kColumnNameArg, "Name of the table column to rename"})
