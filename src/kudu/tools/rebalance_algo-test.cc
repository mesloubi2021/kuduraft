...
bool operator==(const TableReplicaMove& lhs, const TableReplicaMove& rhs) {
  return lhs.table_id == rhs.table_id && lhs.from == rhs.from &&
      lhs.to == rhs.to;
}
...
void ClusterConfigToClusterBalanceInfo(
    const TestClusterConfig& tcc,
  ...
  for (size_t tserver_idx = 0; tserver_idx < tcc.tserver_uuids.size();
    ...
    result.servers_by_total_replica_count.emplace(
        count, tcc.tserver_uuids[tserver_idx]);
  }

  auto& table_info_by_skew = result.table_info_by_skew;
  for (size_t table_idx = 0; table_idx < tcc.table_replicas.size();
       ++table_idx) {
    ...
    for (size_t tserver_idx = 0; tserver_idx < replicas_count.size();
         ++tserver_idx) {
      auto count = replicas_count[tserver_idx];
      info.servers_by_replica_count.emplace(
          count, tcc.tserver_uuids[tserver_idx]);
    }
...
string TestClusterConfigToDebugString(const TestClusterConfig& cfg) {
  ostringstream oss;
  oss << Substitute(
             "TestClusterConfig: $0 tservers, $0 tables",
             cfg.table_replicas.size(),
             cfg.tserver_uuids.size())
      << endl;
  for (const auto& t : cfg.table_replicas) {
    ...
    for (auto i = 0; i < t.num_replicas_by_server.size(); i++) {
      ...
      oss << Substitute(
          "ts $0: $1", cfg.tserver_uuids[i], t.num_replicas_by_server[i]);
    }
    oss << "]" << endl;
  }
  return oss.str();
...
TEST(RebalanceAlgoUnitTest, AlreadyBalanced) {
  ...
  const TestClusterConfig kConfigs[] = {
      ...
      {
          // A configuration which has zero skew cluster-wise, while the
          // table-wise balance has one-off skew: the algorithm should not try
...
TEST(RebalanceAlgoUnitTest, RandomizedTest) {
  ...
  for (auto i = 0; i < num_iters; i++) {
    ...
    // Make sure the rebalancing algorithm can balance the config.
    {
      ...
      int num_moves_ub =
          num_tservers * num_tables * max_replicas_per_table_and_tserver;
      int num_moves = 0;
