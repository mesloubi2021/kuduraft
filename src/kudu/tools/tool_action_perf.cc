...
DEFINE_int32(buffers_num, 2, "Number of mutation buffers per session.");
DEFINE_int32(
    error_buffer_size_bytes,
    16 * 1024 * 1024,
...
bool ValidatePartitionFlags() {
  int num_tablets =
      FLAGS_table_num_hash_partitions * FLAGS_table_num_range_partitions;
  if (num_tablets <= 1) {
    LOG(ERROR) << Substitute(
        "Invalid partitioning: --table_num_hash_partitions=$0 "
        "--table_num_range_partitions=$1, must specify more than one partition "
        "for auto-generated tables",
...
class Generator {
 public:
  ...
  Generator(Mode m, int64_t seed, size_t string_len)
      : mode_(m), seq_(seed), random_(seed), string_len_(string_len) {}
  ...
  for (size_t idx = 0; idx < columns.size(); ++idx) {
    ...
    switch (tinfo->type()) {
      ...
      case DOUBLE:
        ...
        break;
      case DECIMAL32:
        RETURN_NOT_OK(row->SetUnscaledDecimal(
            idx, std::min(gen->Next<int32_t>(), kMaxUnscaledDecimal32)));
        break;
      case DECIMAL64:
        RETURN_NOT_OK(row->SetUnscaledDecimal(
            idx, std::min(gen->Next<int64_t>(), kMaxUnscaledDecimal64)));
        break;
      case DECIMAL128:
        RETURN_NOT_OK(row->SetUnscaledDecimal(
            idx, std::min(gen->Next<int128_t>(), kMaxUnscaledDecimal128)));
        break;
      case BINARY:
...
void GeneratorThread(
    ...
    uint64_t* row_count,
    uint64_t* err_count) {
  const Generator::Mode gen_mode =
      FLAGS_use_random ? Generator::MODE_RAND : Generator::MODE_SEQ;
  ...
  auto generator = [&]() -> Status {
    ...
    RETURN_NOT_OK(session->SetMutationBufferSpace(FLAGS_buffer_size_bytes));
    ...
    for (; num_rows_per_gen == 0 || idx < num_rows_per_gen; ++idx) {
      ...
      RETURN_NOT_OK(
          GenerateRowData(&gen, insert_op->mutable_row(), FLAGS_string_fixed));
...
Status GenerateInsertRows(
  ...
  const auto it = find_if(
      status.begin(), status.end(), [&](const Status& s) { return !s.ok(); });
  if (it != status.end()) {
    return *it;
  }
  return Status::OK();
...
Status TestLoadGenerator(const RunnerContext& context) {
  ...
  RETURN_NOT_OK(
      KuduClientBuilder().master_server_addrs(master_addrs).Build(&client));
  string table_name;
  ...
  cout << "Using " << (is_auto_table ? "auto-created " : "") << "table '"
       << table_name << "'" << endl;
  ...
  sw.start();
  Status status = GenerateInsertRows(
      client, table_name, &total_row_count, &total_err_count);
  sw.stop();
  const double total = sw.elapsed().wall_millis();
  cout << endl
       << "Generator report" << endl
  ...
  if (!status.ok() || total_err_count != 0) {
    ...
    if (total_err_count != 0) {
      ...
      SubstituteAndAppend(
          &err_str, "Encountered $0 write operation errors", total_err_count);
    }
