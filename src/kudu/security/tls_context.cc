...
//
// [1]:
// https://github.com/openssl/openssl/blob/OpenSSL_1_0_1e/ssl/ssl.h#L605-L609
// [2]:
// https://github.com/openssl/openssl/blob/OpenSSL_1_0_1e/ssl/tls1.h#L166-L172
...
DEFINE_bool(
    create_new_x509_store_each_time,
    false,
...
template <>
struct SslTypeTraits<SSL> {
  static constexpr auto kFreeFunc = &SSL_free;
};
template <>
struct SslTypeTraits<X509_STORE_CTX> {
...
Status TlsContext::Init() {
  ...
  // NOTE: 'SSLv23 method' sounds like it would enable only SSLv2 and SSLv3, but
  // in fact this is a sort of wildcard which enables all methods (including
  ...
  // POODLE. We support versions back to TLSv1.0 since OpenSSL on RHEL 6.4 and
  // earlier does not not support TLSv1.1 or later.
  //
...
Status TlsContext::AddTrustedCertificateUnlocked(
  ...
  // minor extra penalty of going under lock
  {
    ...
    // populate it. In the case that two threads try to populate it at the same
    // time, one of the thread's copies will be leaked.
    //
...
#define STORE_UNLOCK(CS) \
  CHECK_EQ(1, X509_STORE_unlock(CS)) << "Could not unlock certificate store"
...
Status TlsContext::GenerateSelfSignedCertAndKey() {
  ...
  // x509v3_cache_extensions. Upon first use of each certificate, this function
  // gets called to parse various fields of the certificate. However, it's
  // racey, so if multiple "first calls" happen concurrently, one call
  // overwrites the cached data from another, causing a leak. Calling this
